...
* Now Playing up-next preview: Navic now adapts Kreate's mini-queue setting as an opt-in Now Playing layout setting. It shows the next queued songs below the progress row, keeps the default player layout unchanged, and opens the full Queue when tapped.
