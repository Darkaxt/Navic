...
* Video concept: Kreate's player exposes a configurable video action, but its implementation is YouTube iframe-specific. Navic uses LidaClips lookup plus Media3 stream playback instead. Matching clips can now run as muted cropped Now Playing backgrounds and the movie toolbar action promotes the clip into the artwork area at the equivalent song-progress percentage.
...
* LidaClips Picture-in-Picture: Navic now exposes a LidaClips Android PiP toggle under Settings -> Integrations -> Music video clips. The standalone Android video surface provides PiP source bounds, uses Android 12+ auto-enter params, and falls back to `onUserLeaveHint` on Android 8-11.
...
* Settings search: Navic now adds a root Settings search field that filters matching preferences into live editable rows, with each result showing its Settings path instead of linking to the owning screen.
...
* Danger-zone confirmations: Navic now adapts Kreate's confirmation pattern for destructive cache/reset operations so Data & Storage danger-zone actions ask before clearing image cache, MusicBrainz metadata/cache entries, pending sync actions, downloads, or the local database.
* Cache usage readouts: Navic now adapts the useful part of Kreate's cache visibility by using a shared human-readable byte formatter for Data & Storage download and image-cache sizes, including small byte/KB cache values.
...
* LidaClips service status/control: Navic now reads LidaClips health/dashboard/control endpoints from Settings -> Integrations -> Music video clips, shows backend health-check failures, active/official/fallback clip counts, recent indexed clips, recent backend sync failures, and sync runtime state, treats degraded health as a reachable connection with diagnostics, and can pause or resume scheduled backend clip sync.
* LidaClips diagnostics display policy: Navic now keeps the LidaClips settings diagnostics testable outside Compose, trims recent clip/failure/status fields consistently, and hides synthetic `[Unknown Artist]` labels from clip and sync-failure summaries without hiding useful track, album, quality, or Lidarr fallback context.
...
* MusicBrainz fallback search: Navic now keeps the playback-only MusicBrainz/Cover Art Archive fallback useful even when Beets/Navidrome did not sync MBIDs for a track. It searches MusicBrainz by recording title and real artist name, skips synthetic `[Unknown Artist]` labels unless a valid synced MBID allows direct lookup, first constrains the search to the local album title when one is available, escapes Lucene query syntax before URL encoding, accepts only high-score recording matches, falls back to the broad title/artist search when the album-specific query misses, and still caches found and missing results locally.
* MusicBrainz metadata cache: Navic now separates playback-time metadata lookup from artwork fallback. Metadata can be fetched and cached for covered songs, Cover Art Archive images remain limited to missing-cover cases, older cached misses without metadata no longer block a metadata refresh, MusicBrainz URL fields in Track Info open only whitelisted public MusicBrainz pages, and safe MusicBrainz recording, linked-work, selected-release, and selected-release-group relation links such as Discogs, Songfacts, Wikipedia, and Wikidata are cached as clickable Track Info rows without separate scraping or API keys.
...
* MusicBrainz metadata detail: Navic now caches and shows MusicBrainz recording, release, and release-group disambiguation text plus release-group type when MusicBrainz returns those fields, making Track Info clearer for remixes, live versions, compilations, and special editions. Older MusicBrainz metadata cache schemas are treated as stale when playback needs metadata so new Track Info fields, including external relation links, are not hidden by previous cache entries.
* MusicBrainz lookup validation: Navic now requires UUID-shaped MBIDs before using synced recording, release, or release-group values for direct MusicBrainz/Cover Art Archive requests. Malformed local tags are ignored for direct lookup while title/artist fallback can still run when enough metadata exists.
* MusicBrainz cache readout and reset: Navic now shows cached MusicBrainz song, artwork, metadata, and missing-result counts under Data & Storage, includes the readout in Settings search as a live value row, and exposes a dedicated Danger Zone action to clear MusicBrainz metadata, Cover Art Archive lookup results, and cached misses without also clearing the normal Coil image cache.
