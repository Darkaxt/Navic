...
* Pause search history: Navic now exposes a Data & Storage setting that hides recent searches and stops recording newly submitted queries while enabled.
...
1. LidaClips playback/session polish
   * The current player is functional and retryable, defaults to Navic music pause/resume while clips play, remembers the last clip position, keeps the screen awake by default, has an opt-in landscape video mode, refreshes stale clip lookup availability, and exposes backend service status/control with health and recent backend failure diagnostics.
