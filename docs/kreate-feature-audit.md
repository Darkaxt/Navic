...
* Now Playing play-button speed label: Navic now adapts Kreate's play-button playback-speed badge. The button remains unchanged at `1.0x` and shows a one-decimal speed label only when playback speed is changed.
...
* LidaClips base URL validation: Navic now shares stricter URL configuration checks across setup, action visibility, prefetch, and repository requests. A configured LidaClips URL must be HTTP(S), include a host, and omit query or fragment parts.
