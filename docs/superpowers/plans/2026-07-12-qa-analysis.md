...
- **Implementation status (2026-07-13):** Fixed and device-validated on the isolated Tranche 4 branch; public release is pending `v1.0.11-iota20`. Production now opens the resolver-owned EPUB once with `ZipFile`, buffers only bounded container/OPF/referenced SMIL/XHTML metadata, streams only manifest-referenced audio through a 64 KiB buffer into versioned `v2` storage, and reuses the existing publication file. The deterministic 56 MiB fixture and Android runtime proof are recorded in `2026-07-13-readaloud-epub-streaming-implementation.md`.
