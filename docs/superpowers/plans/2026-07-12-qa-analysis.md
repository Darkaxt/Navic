...
- **Partial resolution (2026-07-13):** The renderer-generation portion is implemented for `v1.0.11-iota13`: the dispatch ledger survives WebView renderer loss, rebuilds the publication command with the latest observed locator, reuses the stable command ID, and replays only unacknowledged state in deterministic order. On `emulator-5554`, killing renderer PID `2270` left app PID `2197` alive, created renderer PID `2509`, replayed `reader-open-1` at generation 1, restored `OEBPS/Text/capitancebolleta01.xhtml` with `epubcfi(/6/16!/4,/2[sigil_toc_id_4],/22/1:265)`, and received `commandAck(reader-open-1)` without an AndroidRuntime fatal. Process-death restoration for drafts, dialogs, selection, and reconstructed search state remains pending under B15/B24.
