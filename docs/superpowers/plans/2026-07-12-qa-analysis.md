...
- **Direction:** Add a top-level `NOTICES`/`THIRD_PARTY.md` listing foliate-js, pdfjs, and Anx Reader with licenses; surface them in the in-app `AcknowledgementsScreen` (`ui/screens/settings/AcknowledgementsScreen.kt`). Confirm GPL-3.0 is intended (vs AGPL) and that the Anx Reader adaptation's copyleft obligations are stated.
