...
- **Implementation:** Prepared for `v1.0.11-kappa4`: `THIRD_PARTY.md` and exact upstream license files are present; AboutLibraries custom records add all three components to the existing Acknowledgements screen; a structural verifier checks both generated and packaged JSON; CI runs the packaged check before upload.
