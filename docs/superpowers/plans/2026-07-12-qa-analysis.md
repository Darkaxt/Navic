...
- **Resolution (released in `v1.0.11-iota18`):** `ReaderPublicationFormat` now owns one explicit capability matrix: EPUB, AZW3, MOBI, and FB2 support search and media overlays; PDF and CBZ support neither. Search, clear-search, apply/update/clear overlay, Whispersync sidecar/session work, and corresponding dialogs no-op before native state mutation when unsupported. Opening a publication clears transient search/overlay/Whispersync state. Search and Whispersync surfaces use the same matrix, and PDF/CBZ cannot resolve a Whispersync launch attachment. Coordinator and Foliate adapter boundaries independently reject unsupported commands without advancing the bridge command key, and unsupported host events are ignored.
- **Validation:** Capability contracts failed first because the matrix and predicates did not exist. The B3 owner batch then passed 156/156 with zero failures, errors, or skips. A 195-test runtime/chrome batch had 20 baseline failures on clean public `master`; the sole branch-only failure was an obsolete source assertion requiring an unconditional search dialog, which was updated to require the capability guard and passed. Source vendor 30/30, verifier self-test, attribution, Android debug assembly, packaged vendor 30/30, and packaged attribution passed. Workflow `29261226228` built and released Android successfully with both iOS jobs skipped. The 46,225,284-byte public APK has SHA-256 `62e210ab7536e8448366a141970bcc07ef0a67d3dd09de7706856bbb42ed410c`, matching GitHub's digest; it passed v2 signature with the established certificate, `545/iota18` metadata, all 30 vendor hashes, attribution, and an in-place `iota17` upgrade. Public `MainActivity` resumed as PID `15663` with no targeted AndroidRuntime/MediaController/Koin/Room error logs.
...
- **Resolution (released in `v1.0.11-iota16`):** `ReaderWebRuntime.configure()` now uses `WebSettings.LOAD_DEFAULT` and no longer calls process-global `webView.clearCache(true)`. The secure appassets origin, file/content access hardening, bridge ownership, load URL, managed publication storage, and renderer replay contract are unchanged. No conditional invalidation state, versioned URL scheme, user-facing control, timeout, or iOS work was added.
- **Validation:** The focused contract failed first against `LOAD_NO_CACHE`, then 5/5 immediate cache/entrypoint/viewport/acknowledgement/generation tests and the integrated 185/185 reader owner suite passed. JavaScript syntax, command acknowledgement 1/1, page-turn model 15/15, reader smoke/trace smoke, source vendor 30/30, tamper self-test, attribution, debug/reader-dev assembly, and both packaged governance gates passed. On `emulator-5554`, killing renderer PID `7216` kept app PID `7074` alive, created renderer PID `7484`, and generation 1 restored `OEBPS/Text/1.html` at `epubcfi(/6/2!/4,/2,/126)` before acknowledging `reader-open-1`; AndroidRuntime emitted no fatal error. Release workflow `29255331455` built and published Android successfully with all iOS jobs skipped. The public APK is 46,225,284 bytes with SHA-256 `1d630864e5aff35b08e35fbe991eb5e266e323055042966b206a7a3f24340f6f`, matching GitHub's digest; it passed v2 signature, `543/iota16`, all 30 vendor, attribution, and in-place `iota15` upgrade/startup checks.
...
- **Resolution (2026-07-13):** Released in `v1.0.11-iota20`. Production now opens the resolver-owned EPUB once with `ZipFile`, buffers only bounded container/OPF/referenced SMIL/XHTML metadata, streams only manifest-referenced audio through a 64 KiB buffer into versioned `v2` storage, and reuses the existing publication file. The deterministic 56 MiB fixture and Android runtime proof are recorded in `2026-07-13-readaloud-epub-streaming-implementation.md`. GitHub Actions run `29271201032` succeeded with iOS skipped; the public APK passed digest, v2 signature, embedded `547/iota20`, vendor/attribution, and in-place `iota19` upgrade/cold-start verification.
...
- **Resolution (released in `v1.0.11-iota19`):** `ReaderProcessStateViewModel` retains only a bounded, publication-keyed `SavedStateHandle` snapshot: current dialog, typed/submitted search intent, semantic selection, and selection-note anchor/text/draft. Durable publication/locator data remains in existing persistence; coordinator, WebView/runtime hosts, search results, TOC, popups, Whispersync runtime state, playback plans, command acknowledgements, and coroutine scopes are reconstructed. Input changes are captured immediately, restoration validates publication identity and format capabilities, submitted EPUB search is reissued after open, and intentional reader exit clears the snapshot.
- **Validation:** The process-state owner suite passed 154/154 with zero failures or errors, including 8 dedicated recovery tests. On `emulator-5554`, an EPUB search dialog restored the exact typed and submitted `recoveryprobe` query after Android reported `mHaveState=true`/`state=STOPPED`, process PID replacement, and activity restoration; search was reissued to completion. PDF and CBZ each restored a one-activity reader task after PID replacement, emitted `publicationReady` again, exposed no Search UI, issued no search command, and produced no AndroidRuntime fatal.
...
- **Resolution (released in `v1.0.11-iota17`):** Forced Android WebView debugging is now represented by synchronized, idempotent ownership leases instead of a process-lifetime Boolean setter. `MainActivity` acquires with the compile-time `NAVIC_READER_DEV` value before Compose mounts and closes its exact lease in `onDestroy`; overlapping activity owners cannot disable one another, and the final release reapplies the normal false default. The release build explicitly compiles `NAVIC_READER_DEV=false`, while only `readerDev` compiles it true. The existing user-controlled developer option remains independent.
- **Validation:** The new host contract failed first because `ReaderWebDebuggingForceRegistry` did not exist, then the B23 owner suite passed 138/138 with zero failures, errors, or skips. A broader 197-test reader batch had only two unrelated asset/harness source-shape failures, both reproduced unchanged on a temporary clean `fork/master` baseline. Source vendor 30/30, tamper self-test, attribution, debug/reader-dev assembly, and both packaged governance gates passed. Generated BuildConfig values were false for debug and true for reader-dev. On `emulator-5554`, reader-dev PID `8739` exposed `webview_devtools_remote_8739` while the activity owned the lease; normal back/task exit retained the process but logged `WebView debugging enabled=false`. Error-level AndroidRuntime/MediaController/Koin/Room checks were clean. Release workflow `29257940670` built and published Android successfully with both iOS jobs skipped. The 46,225,284-byte public APK has SHA-256 `a8bf54f5ff598e22da416dd9850797b811c42594091b94b13d451391da9aa81d`, matching GitHub's digest; it passed v2 signature, `544/iota17`, all 30 vendor, attribution, and in-place `iota16` upgrade/startup checks without a forced-debug enable log.
...
- **Resolution (completed in `v1.0.11-iota19`):** The renderer-generation portion shipped in `v1.0.11-iota13`: the dispatch ledger survives WebView renderer loss, rebuilds the publication command with the latest observed locator, reuses the stable command ID, and replays only unacknowledged state in deterministic order. On `emulator-5554`, killing renderer PID `2270` left app PID `2197` alive, created renderer PID `2509`, replayed `reader-open-1` at generation 1, restored `OEBPS/Text/capitancebolleta01.xhtml` with `epubcfi(/6/16!/4,/2[sigil_toc_id_4],/22/1:265)`, and received `commandAck(reader-open-1)` without an AndroidRuntime fatal. `v1.0.11-iota19` added the bounded `SavedStateHandle` boundary described in B15, restoring drafts/dialog/selection/search intent while reconstructing all runtime and derived state; the Android EPUB/PDF/CBZ recreation matrix passed without a fatal or unsupported search dispatch.
