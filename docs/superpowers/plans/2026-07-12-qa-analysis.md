...
- **Resolution (2026-07-13 candidate):** `ReaderWebRuntime.configure()` now uses `WebSettings.LOAD_DEFAULT` and no longer calls process-global `webView.clearCache(true)`. The secure appassets origin, file/content access hardening, bridge ownership, load URL, managed publication storage, and renderer replay contract are unchanged. No conditional invalidation state, versioned URL scheme, user-facing control, timeout, or iOS work was added.
- **Validation:** The focused contract failed first against `LOAD_NO_CACHE`, then 5/5 immediate cache/entrypoint/viewport/acknowledgement/generation tests and the integrated 185/185 reader owner suite passed. JavaScript syntax, command acknowledgement 1/1, page-turn model 15/15, reader smoke/trace smoke, source vendor 30/30, tamper self-test, attribution, debug/reader-dev assembly, and both packaged governance gates passed. On `emulator-5554`, killing renderer PID `7216` kept app PID `7074` alive, created renderer PID `7484`, and generation 1 restored `OEBPS/Text/1.html` at `epubcfi(/6/2!/4,/2,/126)` before acknowledging `reader-open-1`; AndroidRuntime emitted no fatal error.
