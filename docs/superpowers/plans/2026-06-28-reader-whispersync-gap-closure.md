...
- 2026-07-02 cadence override: iteration artifacts are local debug builds or readerdev installs only. "Release" means a GitHub tag, prerelease/release, public APK upload, or physical-phone candidate. Final/public APK releases are reserved for feature slices or major fixes that have already been implemented, deployed in debug/readerdev, and passed the relevant host, harness, emulator, and source checks; historical completed-stage release notes below are evidence records, not instructions to publish every new patch.
- Any request to publish from this plan must name the completed feature/fix and cite the validation evidence that makes it worth physical-device acceptance. If the evidence is only a microfix, a diagnostic hardening change, or one green isolated probe, keep working in readerdev.
...
- `C:\Users\darka\Documents\Projects\Android\Navic\bindery-debug.env` contains the credentials needed for Bindery-scoped ebook/audiobook integration validation. That is the authoritative path for `darkaxt.navic.readerdev`, direct reader launches, Whispersync sidecar fetches, paired audiobook manifest/resource resolution, page-scoped headset playback, audio-follow overlays, and exact companion progress checks.
- Public `darkaxt.navic` login state and `navic-release-login.env` are only required for signed release package/app-shell proof on a fresh profile. They are not blockers for ebook/audiobook implementation or readerdev validation, and they must not be used as an excuse to stop Whispersync work.
- The generated-cover metadata gap only blocks Stage 8N's Bindery-owned fullscreen cover variant work. It does not block the current Whispersync enjoyment matrix, sidecar/audio integration, or Komikku/Anx reader-shell work.
...
- GREEN/READERDEV-FOLLOWUP: current-source readerdev was rebuilt and installed on `emulator-5554`, reached `publicationReady` for production book `3809` (`captures\reader-dev\reader-dev-20260630-144505.png`), and the paired Whispersync enjoyment gate passed in `captures\reader-whispersync-enjoyment\stage5c3-whispersync-enjoyment-20260630-144538`.
- GREEN/KOMIKKU-MATRIX-FOLLOWUP: `.\scripts\adb-reader-komikku-matrix.ps1 -Package darkaxt.navic.readerdev -DeviceSerial emulator-5554 -ExpectedVersionName v1.0.11-theta25 -PrepareReaderLaunch -IncludeCoverChecks -ContinueOnFailure -ArtifactRoot captures\reader-komikku-matrix\stage6f-current-source-theta25-20260630` passed all 12 rows; `reader-matrix-failures.txt` reports `No matrix failures.`
...
- Preferred Bindery contract: expose generated reader covers as explicit non-thumbnail assets, for example `type="readerShellCover"` or one of the already-supported fullscreen/extended/expanded/shell-cover fields. Each generated asset should carry `href`/`url`, pixel dimensions or `aspectRatio`, a source cover hash, and a generator/style version. The stable cache identity should be derived from `bookId + sourceCoverHash + targetRatio + generatorVersion` so Navic can treat the URL as a durable server-side derivative instead of a runtime AI request.
...
- Navic must not infer reader-shell covers from ordinary thumbnails or `rel="cover"` images, and must not call an AI/image-generation service at reader open time. If no generated reader-shell asset is present, the current EPUB-extracted/native fallback remains authoritative.
...
- RED/HOST-FIRST: `BinderyBookVersionPolicyTest.ebookVersionRowsAcceptExplicitReaderShellCoverTypeFromBindery` failed while an explicit Bindery image with `type="readerShellCover"` and ordinary `rel="cover"` was ignored.
- FIXED/TYPE-CONTRACT: `BinderyFullscreenCoverPolicy.kt` now treats explicit generated/fullscreen/shell-cover type tokens, including `readerShellCover`, as reader-shell cover assets while preserving the rejection of ordinary image MIME types and plain `rel="cover"` thumbnails.
- GREEN/FOCUSED-HOST: the forced rerun of `BinderyBookVersionPolicyTest.ebookVersionRowsAcceptExplicitReaderShellCoverTypeFromBindery` passed after the type-token policy change.
- GREEN/BROADER-HOST: `.\gradlew.bat --no-daemon --console=plain :composeApp:testAndroidHost --tests "paige.navic.ui.screens.bindery.BinderyBookVersionPolicyTest"` passed after adding the explicit type contract.
- GREEN/AGGREGATE: `.\gradlew.bat --no-daemon --console=plain :composeApp:testAndroid` passed after the explicit type contract.
...
- EXTERNAL-GATE/BINDERY-CONTRACT: the proposed Bindery-side implementation should generate/cache one or more reader-shell cover derivatives server-side and expose them as explicit generated cover assets. Navic is already prepared to select the closest aspect variant and authenticate/cache the chosen URL locally, but cannot prove the end-to-end path until Bindery emits live metadata for a book.
- GREEN/FOCUSED-HOST-REFRESH: after the Bindery-side generated cover discussion, the guard batch `ReaderRuntimeCommonChromeTest.commonWhispersyncPlaybackControlBlendsIntoPageInsteadOfRenderingChromePill`, `BinderyBookVersionPolicyTest.ebookVersionRowsChooseClosestGeneratedFullscreenCoverVariantForReaderAspect`, `BinderyBookVersionPolicyTest.regularCoverImagesDoNotBecomeFullscreenShellCoverRoutes`, and `BinderyReaderPublicationResolverTest.cachesExternalBinderyShellCoverAsLocalAssetUriForNativeCoverSurface` passed. This refresh proves the current branch still has a low-opacity page-native Whispersync headset and the client-side generated/fullscreen cover contract is ready for live Bindery assets.

Closure:
- [x] Add a failing host guard for generated fullscreen cover variants.
- [x] Preserve single fullscreen-cover URL and rel support.
- [x] Support explicit generated reader-shell cover `type` tokens such as `readerShellCover`.
