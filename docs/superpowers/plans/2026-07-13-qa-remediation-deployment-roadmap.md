...
- `ReaderJavascriptBridge.deactivate()` prevents a retired JavaScript interface from decoding or forwarding later messages. The keyed host generation owns one bridge, an `AtomicReference` to its exact WebView, and an `AtomicBoolean` disposal gate.
- Bridge callbacks post only to that generation's view and re-check the disposal flag, generation number, and exact global-view identity on the UI thread. There is no fallback that handles an event after the generation view has disappeared.
...
- After rebasing the concurrent ebook-animation commit, managed-storage, resolver, font, Storyteller, bridge, host, controller, and coordinator tests passed 184/184 with zero failures, errors, or skips; 10/10 exact storage UI/source assertions also passed. JavaScript syntax, Chromium command acknowledgement, page-turn model 15/15, reader smoke/trace smoke, source vendor 30/30, tamper self-test, attribution, debug/reader-dev assembly, and both packaged vendor/attribution gates passed. The broader mutable `ReaderRuntimeCommonChromeTest` still has 18 unrelated ebook source-shape failures against the concurrent animation surface; none touches B22-owned files or assertions.
...
- RED failed because the registry was unresolved. The focused B23 owner suite then passed 138/138 with zero failures, errors, or skips. A broad 197-test reader run had two unrelated failures (`androidReaderEntrypointKeepsRuntimeMethodGroupsSplit` and `adbWebViewEvalHelperReportsDecorativePaperTextureState`), both reproduced unchanged on a temporary clean public-master baseline whose worktree was removed.
