...
- Release cadence is debug-build-first and public-release-last. Intermediate reader slices, minor fixes, diagnostics, and unproven animation changes must use local debug/readerdev APKs on the emulator plus local harness/ADB validation only. A public GitHub APK release is allowed only after a coherent feature or major user-visible fix is fully implemented, deployed in debug/readerdev, validated through its plan gates, committed, and ready for physical-device acceptance testing.
...
- Post-theta31 physical feedback found a real drag commit regression: while settled on Chapter 1 page 1/global `5/140`, dragging toward the next page previewed page 6 content but finger-up committed page 7 as the new page 6. Later production-fixture evidence narrowed the failure to two separate causes: synthetic preview iframes could show text that did not match Foliate's committed paginator geometry, and Android could dispatch a redundant native page command immediately after the live snap. The current Stage 9H debug slice fixes same-section drags by reusing Foliate's live strip during preview, snapping that strip on release, and suppressing the redundant native page command. `epub-native-drag-single-commit` guards the original late-section red target plus nearby pages; texture feel and readerdev/device validation remain separate gates.
...
- The current debug source extends Plan H after H2: section-boundary fallback navigation now remains part of the active page-turn transaction until the adjacent section posts, Standard live-strip preview is accepted by the harness without synthetic iframe assumptions, and Curl mode is explicitly excluded from the live-strip shortcut. Browser harness proof covers `epub-texture-frontmatter-transition`, `epub-texture-page-turns`, `epub-native-drag-standard-no-curl`, and the original `epub-native-drag-single-commit` target. This is not a public release candidate until readerdev/emulator and physical acceptance gates confirm the feel.
- The current curl animation is not Turn.js. It is a custom, mockup-derived snapshot overlay in `navic-reader-page-turns.js` that creates `data-navic-page-curl-sheet` roles, snapshot iframes, and drag-scoped CSS variables. Turn.js is a jQuery flipbook engine that owns a page container and single/double page display model, so importing it would be a separate rendered-page adapter project rather than a drop-in fix for Foliate's active EPUB renderer. Standard mode remains the default; curl remains opt-in/experimental.
- Plan J adds rendered rotation coverage for the tablet landscape layout complaint. The new `epub-landscape-rotation-layout` harness opens the production EPUB in portrait, rotates the same session to `1974x1232`, and requires the active Foliate renderer/content document to reflow as a wide two-column landscape spread. Current source passes this guard with `maxColumnCount=2`, renderer `1974x1232`, content body width `1737.125`, and probe column width `809.34375`. This is debug harness evidence only; physical release-device layout feel remains separate.
- Plan K closes one debug-reproduced texture transition class: Foliate can move text by a renderer stride narrower than the full reader viewport, while the paper and border texture slots are full-surface layers. The runtime now scales texture scroll deltas from `renderer.size` to the full viewport surface. The Hobbit fixture at `1536x2048` target global page 11 now reaches live current texture `x=-686` and release snap `x=-1536` before the committed texture update; production Bindery book `3809` file `426` at `1974x1232` also passes the same debug gate. This is still not a public release trigger; physical texture feel, border/shadow intensity, and page-turn perception remain separate acceptance items.
- Plan L closes the objective page-number font-parity class in debug/browser evidence. The organic root page-number layer now keeps its small in-book placement and `# / #` label while applying the same configured reader font weight, letter spacing, and word spacing as visible EPUB prose. The `epub-page-number-font-parity` row passes on the Hobbit fixture and production Bindery book `3809` file `426` with the Dys stack plus `font-weight=500`, `letter-spacing=2px`, and `word-spacing=6px` shared between prose and page number. Physical visual judgment still remains part of final candidate acceptance.
...
- Validate the progress rail fixes on a clean release candidate or the exact device/package where the user saw `10 / 12`, `2 / 4`, and page-1 rail-button failures.
  ...
  The 2026-07-02 current-source readerdev gate now closes the emulator harness failure mode for current-chapter endpoint taps: the matrix clamps exact slider-bound taps inside the UI node and strips Whispersync follow state during `-OnlyRailEndpointChecks`; Chapter 1 validated `0/9` and `8/9` in `captures\reader-komikku-matrix\plan-m-current-rail-endpoints-fixed`. Keep physical/final-candidate validation open; do not publish a public APK only for this harness stabilization.
...
- Debug-closed on 2026-07-02: renderer-stride-to-full-surface texture scaling now prevents the old partial-surface snap/swap class when Foliate's page stride is narrower than the viewport. Keep physical-device texture feel, border/shadow intensity, and section-boundary perception open until a final candidate pass.
- Debug-closed on 2026-07-02: organic page-number typography now follows the selected reader font stack plus weight, letter spacing, and word spacing in computed browser evidence. Keep physical visual judgment open for final candidate acceptance.
...
5. Continue texture movement polish as the remaining drag-feel slice only after debug gates reproduce a concrete class of failure; the renderer-stride/full-surface mismatch is debug-closed, while physical texture feel and border/shadow intensity still need release-device validation.
