...
- Phase 5 dirty emulator evidence proves the selection payload can reach Android with `footnote=true`, CFI, context text, and bounds, and that the native Komikku-style `Highlight`, `Copy`, and `Note` action overlay appears after shell-cover dismissal. Highlight has a repeatable smoke gate through `applyHighlights` + `annotationDrawn`; Copy has a repeatable node-tap smoke gate that reaches the native clipboard boundary (`Reader selection copied length=31`). Note is now dirty-emulator verified through a DevTools-created selection plus native UI taps: `Reader selection note save length=10`, `applyHighlights(... notes=1)`, and `annotationDrawn` were captured on eta76. Clean release and user-driven normal-text selection validation remain open.
...
- Validate the host-verified selection action UI on a clean release APK and with user-driven text selection: dirty-emulator automation now proves a DevTools-created selection surfaces Highlight/Copy/Note without opening reader chrome, Copy reaches the clipboard, Highlight/Note render as annotations, and footnote selections keep the Anx payload fields.
