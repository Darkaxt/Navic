...
- Phase 5 dirty emulator evidence now proves the selection payload can reach Android with `footnote=true`, CFI, context text, and bounds, and that the native Komikku-style `Highlight`, `Copy`, and `Note` action overlay appears after shell-cover dismissal. The Highlight button has a repeatable smoke gate using `selection-payload` plus `-PostProbeTapFraction` and `-RequireReaderEngineCommand applyHighlights`; Copy and Note still need button-level behavior gates.
