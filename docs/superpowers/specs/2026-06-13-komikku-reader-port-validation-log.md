...
- GREEN: `node --check composeApp\src\androidMain\assets\reader\navic-reader-pagination.js` and `node --check tools\reader-harness\src\run-reader-harness.mjs` passed.
- GREEN: `git diff --check` passed.
- GREEN/READERDEV: `.\scripts\install-reader-dev.ps1 -DeviceSerial emulator-5554 -EnvFile "C:\Users\darka\Documents\Projects\Android\Navic\bindery-debug.env" -PublicationUrl "https://bindery.remaxku.eu/book/3809" -ResourceHref "https://bindery.remaxku.eu/api/v1/book/3809/file?bookFileId=426" -BookId 3809 -Title "Bastille vs. the Evil Librarians" -Kind Ebook -Format EPUB -SkipNativeShellCover` rebuilt, installed, launched `darkaxt.navic.readerdev`, and reached `publicationReady`.
- GREEN/ADB PROBE: `node tools\reader-harness\src\adb-webview-eval.mjs --package darkaxt.navic.readerdev --device emulator-5554 --probe page-number-spread-layout` passed on the visible extras-launched reader activity with `viewportWidth=1974`, `viewportHeight=1232`, `maxColumnCount=2`, `slotCount=2`, left slot `7 / 168` at `centerX=493.5`, and right slot `8 / 168` at `centerX=1480.5000839233398`.
- GREEN/SMOKE: `.\scripts\adb-reader-smoke.ps1 -Package darkaxt.navic.readerdev -DeviceSerial emulator-5554 -NoLaunch -ArtifactDir captures\reader-dev\page-number-spread-layout-smoke-nolaunch -ReaderDevtoolsProbe page-number-spread-layout -RequireNoReaderConsoleErrors -CaptureWaitSeconds 1` passed against the already-open readerdev activity. Artifacts: `captures\reader-dev\page-number-spread-layout-smoke-nolaunch`.
- GREEN/VISUAL: final screenshot `captures\reader-dev\page-number-spread-layout\screen-final.png` shows dual-page layout with independent organic page labels under each page, not one shared centered label.
- GREEN/CURRENT HEAD: after the local `v1.0.11-theta42` version bump, `install-reader-dev.ps1` was rerun from current HEAD. `adb-reader-smoke.ps1 -NoLaunch -ReaderDevtoolsProbe page-number-spread-layout` passed with `versionCode=470`, `versionName=v1.0.11-theta42`, `slotCount=2`, left slot `7 / 168`, and right slot `8 / 168`. Artifacts: `captures\reader-dev\page-number-spread-layout-theta42-smoke`.
- GREEN/CURRENT VISUAL: `captures\reader-dev\page-number-spread-layout-theta42-smoke\screen-final-after-probe.png` shows the two organic labels rendered under their respective pages.
- INVALID/PIPELINE: an earlier smoke run without `-NoLaunch` relaunched the package through the generic launcher intent and returned to the Books grid. That run is not counted as reader validation; readerdev checks that depend on explicit extras must use `install-reader-dev.ps1` for launch and `adb-reader-smoke.ps1 -NoLaunch` for collection.
