...
## Context Resume Stop Sign

This section exists specifically because compaction/restart events have caused the emulator procedure to be lost. On every resumed or compacted session that touches the reader, read this section before reading old chat summaries, choosing a fix, or editing code.

Hard rule:

- If the current worktree contains major reader code/asset/script changes and the latest dated section at the bottom of this document does not record emulator validation for that exact worktree/APK state, the next task is emulator validation, not another implementation slice.
- A host test, desktop browser harness, manual screenshot, or "it should be obvious" reasoning is not evidence for Android reader UI/input/progress behavior.
- Raw manual `adb shell input ...` commands are diagnostics only. They do not replace the scripted matrix.
- If the emulator launch, Bindery seed, reader-dev open, or matrix script is broken, that script/environment failure becomes the current task. Fix the validation path with a guard before returning to reader features.

Mandatory resume preflight:

1. Read `Hard Procedure: Emulator Verification Gate` in this document.
2. Read the latest dated validation section at the bottom of this document.
3. Run `adb devices` and pick the intended emulator/device serial explicitly.
4. Check installed `darkaxt.navic.readerdev` evidence on that serial: `versionName`, `versionCode`, and `lastUpdateTime`.
5. Compare that installed evidence with the current reader-path diff and APK timestamp. If stale or ambiguous, rebuild/install/open reader-dev with `scripts\install-reader-dev.ps1`.
6. Run `scripts\adb-reader-komikku-matrix.ps1` with `-DeviceSerial`, `-ExpectedVersionName`, `-NoLaunch`, `-IncludeCoverChecks`, and a fresh artifact root.
7. Append the commands, artifact root, installed APK evidence, pass/fail summary, and next required fix here before continuing implementation.
...
6. If the comparison shows that an existing feature works but is not faithful, stop treating that feature as a valid baseline. Redesign it from the reference contract before polishing it, building dependent behavior on it, or publishing it as a release candidate.
7. Verify the changes in the emulator after every major reader change by following the Emulator Verification Gate below. A host test, local browser harness, or manual screenshot is not enough for Android reader UI/input/progress claims.

## Hard Procedure: Emulator Verification Gate

This section is mandatory compaction-recovery procedure. If context has compacted, restarted, or resumed, read this section before claiming any Android reader UI/input/progress behavior is verified.

### Compaction Survival Rule

The emulator gate is not a final QA flourish. It is the loop boundary for reader work.

If the previous assistant turn, compaction summary, git diff, or dated validation log shows a major reader change that has not yet been validated in the emulator, the next task is the emulator gate. Do not continue implementing another reader feature, do not polish UI, and do not start a new workaround until one of these is recorded in this document:

- `Emulator validated`: the reader-dev APK was built/installed from the current worktree, the scripted matrix ran on the selected emulator/device, artifacts were inspected, and the pass/fail result was appended here.
- `Emulator blocked`: the emulator, APK install, launch script, matrix script, or Bindery seed path failed. The blocking script/environment failure becomes the current task and must be fixed with a guard before returning to reader code.
- `Host/local only`: the change is intentionally limited to a host harness, pure documentation, or non-Android diagnostic path. This label cannot be used for reader shell, Android input, progress, pagination, texture, PDF, EPUB rendering, or settings behavior.

After any major reader code or asset edit, the next dated section appended to this document must be an emulator result or an emulator blocker. A later compaction must be able to answer three questions without reading chat:

1. Which exact APK/version/timestamp was installed?
2. Which exact emulator/device serial ran the matrix?
3. Which artifact root contains the screenshots, logs, summary, and failures?

If those three answers are missing, the work is not emulator-verified and no Android reader behavior can be called fixed.

### Required Loop After Every Major Reader Change

Run this loop before moving to the next reader implementation slice:

1. Build/install/open reader-dev with the repo script, targeting the selected emulator/device serial.
2. Wait for the script's foreground and `publicationReady` checks; do not accept a splash, library, login, or unloaded WebView screenshot.
3. Run `adb-reader-komikku-matrix.ps1` with `-NoLaunch`, `-DeviceSerial`, `-ExpectedVersionName`, `-IncludeCoverChecks`, and the run-specific artifact root.
4. Inspect the baseline screenshot/window hierarchy first. If it is not the reader, mark the run invalid and rerun setup.
5. Inspect `reader-matrix-summary.csv`, `reader-matrix-failures.txt`, relevant screenshots, and relevant diagnostics.
6. Append the command, artifact root, installed APK evidence, pass/fail summary, invalid-run notes, and next required fix to this document.
7. Only then continue with implementation.

This loop applies even when the change was "obvious", locally testable, or already covered by a Gradle/Node/browser harness. Those checks are useful guards, but they do not replace the emulator matrix for Android reader behavior.

### Compaction Recovery Checklist

At the start of any resumed or compacted session that touches the reader, do this before making product claims or starting a new workaround:

1. Read this section and the latest dated validation section at the bottom of this document.
2. Check the current worktree for reader-path changes and identify whether the touched files require the emulator gate.
3. Check connected devices with `adb devices`. If more than one device is attached, pick the intended target and pass `-DeviceSerial` to every repo validation script.
4. Check whether `darkaxt.navic.readerdev` is installed on that same serial and record its `versionName`, `versionCode`, and `lastUpdateTime`.
5. Compare `lastUpdateTime` and the APK `LastWriteTime` against the changed reader source/assets. If the emulator/device is stale, rebuild and reinstall before testing.
6. Run the repo script path below. Do not substitute manual `adb shell input ...` gestures for the matrix.
7. Append the artifact root, summary, failures, and any remaining manual/user observations to this document before continuing to the next reader fix.

If any step above cannot be completed because a script is broken, the script failure becomes the current task. Add or update a guard for the script and fix it before continuing with reader code.

### What Counts As A Major Reader Change

Run the emulator gate after any change that touches one of these areas:

- Reader shell layout, app bars, bottom bar, settings dialog, chapter navigator, page indicator, cover surface, progress rail, menu visibility, or visual parity with Komikku.
- Native input ownership, tap zones, long press, drag/swipe, image/link interaction, cover gestures, or WebView/PDF touch routing.
- EPUB/PDF page turns, pagination profile, page numbers, texture movement, paper/edge textures, theme tinting, hyperlink behavior, or content bridge events.
- Reader dev launch, Bindery reader seeding, WebView debugging, APK packaging, or validation scripts.

Small documentation-only edits do not require an emulator run. Any code or asset edit in the reader path does.

### Required Verification Path

Do not manually stitch together raw `adb` commands when the repo script can perform the same validation. The normal path is:

```powershell
adb devices
.\scripts\install-reader-dev.ps1 -EnvFile 'C:\Users\darka\Documents\Projects\Android\Navic\bindery-debug.env' -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -Capture
```

`install-reader-dev.ps1` is responsible for the launch readiness gate. When it launches a direct reader target, it must clear stale logcat, wait until `darkaxt.navic.readerdev` is the focused Android window, then wait for the reader bridge `publicationReady` event before capturing or handing the emulator to the matrix. A screenshot captured before `publicationReady` is invalid even if the package is foreground, because it can still be the Navic splash or an unloaded WebView.

Then run the Komikku reader matrix against the freshly built APK:

```powershell
$expectedVersionName = (Select-String -Path 'androidApp\build.gradle.kts' -Pattern 'versionName = "([^"]+)"').Matches.Groups[1].Value
.\scripts\adb-reader-komikku-matrix.ps1 -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -ExpectedVersionName $expectedVersionName -ArtifactRoot 'captures\reader-komikku-matrix\<run-id>' -NoLaunch -IncludeCoverChecks -ContinueOnFailure
```

Replace `emulator-5554` with the selected serial from `adb devices` when validating another emulator or phone. If more than one device is attached and no serial is passed, the run is invalid even if individual ADB commands appear to work.

Do not pass `-ApkPath` to the matrix after `install-reader-dev.ps1` has opened the reader. Passing `-ApkPath` without `-NoLaunch` makes the matrix reinstall and monkey-launch the app, which can land on the Books library instead of the reader. A matrix run from the Books library is invalid for reader input/progress claims, even if the APK version and timestamp are current.

The baseline matrix step must prove the active UI is the reader surface. Inspect the baseline screenshot/window hierarchy before interpreting gesture failures. If the baseline shows the Books grid, login page, library navigation, or any non-reader surface, record the run as invalid and rerun the setup/matrix path; do not debug reader behavior from that artifact.

If the relevant change touches PDF behavior, add `-IncludePdfChecks`. If the relevant change is PDF-only, use `-OnlyPdfChecks`.

The expected version must match `androidApp/build.gradle.kts`. If `versionName` has not changed, the proof must include `lastUpdateTime` from the matrix artifacts or `adb shell dumpsys package darkaxt.navic.readerdev`, and that timestamp must be newer than the changed source/assets.

### Evidence Required Before Claiming Emulator Verification

An emulator/device claim is allowed only when all of these are true:

- `readerDev` was built from the current worktree after the changed reader files.
- The installed package is `darkaxt.navic.readerdev`, not the public release package.
- The installed package `lastUpdateTime` is newer than the changed source/assets or the APK `LastWriteTime` used for install.
- The validation script produced an artifact root under `captures/reader-komikku-matrix/...` or `captures/reader-dev/...`.
- If the setup path launched a direct ebook, the setup transcript shows the foreground wait and `publicationReady` wait before the screenshot or matrix.
- `reader-matrix-summary.csv`, `reader-matrix-failures.txt`, relevant screenshots, and relevant `reader-diagnostics-summary.txt` files were inspected.
- The result was appended to this document with the command, artifact root, pass/fail status, and remaining failures.

If these artifacts do not exist, the behavior was not emulator-verified. Say "host/local verified only" or "not emulator verified" instead of claiming that the emulator/device was tested.

Manual screenshots, local browser harnesses, isolated Gradle tests, and raw ADB taps are useful diagnostics, but they are never acceptance evidence for Android reader UI/input/progress changes by themselves. They must point back to a matrix artifact or be recorded as "diagnostic only".

### Script Failure Rule

If `install-reader-dev.ps1`, `adb-reader-smoke.ps1`, or `adb-reader-komikku-matrix.ps1` fails because of script plumbing, parameter binding, stale assumptions, or broken artifact capture, fix the script first with a guard. Do not replace it with a one-off manual ADB sequence except to diagnose the script failure.

Raw `adb` commands are allowed for diagnosis only:

- checking connected devices,
- confirming package/version/timestamp,
- inspecting foreground activity,
- collecting additional logs for a failed scripted step.

They are not a substitute for the scripted matrix.

### Stale APK Rule

If the emulator shows old behavior, first prove whether the installed APK is stale:

```powershell
adb shell dumpsys package darkaxt.navic.readerdev | findstr /i "versionName versionCode firstInstallTime lastUpdateTime codePath"
Get-Item 'androidApp\build\outputs\apk\readerDev\Navic.apk'
```

If the source or asset files are newer than the installed package, rebuild/install before testing. Do not debug product behavior from a stale emulator install.
...
There is no "working exception" for the reader. A feature that appears functional but is not faithful to its source authority must be redesigned, not polished, stabilized, or released as complete. If a feature is not faithful, it must be redesigned even if it is supposed to be working. This includes UI that only resembles Komikku, input handling that works by bypassing Komikku's ownership model, progress/page logic that gives plausible numbers without the reference chapter/page contract, and EPUB/PDF engine behavior that works through Navic-specific shortcuts instead of the Anx/Foliate capability boundary. The redesign requirement applies even when the non-faithful implementation is already shipped in a test release.

Redesign is mandatory as soon as non-faithfulness is identified. Do not keep a non-faithful feature on the grounds that it is currently usable, faster to patch, less risky to leave alone, or already validated by old Navic tests. The next implementation slice must either replace it with a faithful reference-derived design or explicitly mark it as a `Temporary adapter` with the reference gap, removal path, and user-accepted reason recorded here. Without that record, the feature status is `Failing`.
...
- `Failing`: the feature works locally or on device but is not faithful to the reference source. This status requires redesign before it can be treated as a bugfix, milestone, release candidate, or foundation for more features.

## Non-Faithful Working Feature Blocker

Working status is diagnostic evidence, not acceptance evidence. If the device, emulator, or host harness shows that a reader feature "works" but the feature is not faithful to the named Komikku or Anx/Foliate source authority, the result only proves what needs to be redesigned. It does not prove that the feature is acceptable.

A non-faithful feature cannot be used as the foundation for new work. Do not build polish, settings, release notes, dependent UI, or new engine behavior on top of a feature marked `Failing`. The next slice touching that area must either redesign the feature from the reference source or record it as a `Temporary adapter` with the exact missing reference behavior, removal path, and user-accepted reason.

No release candidate may be created from a slice whose touched reader behavior is still `Failing`. Passing Gradle tests, passing host harness checks, or looking usable on Android is not enough if the touched behavior is known to diverge from Komikku's UI/input/shell contract or Anx/Foliate's EPUB/PDF engine contract. Release candidates require every touched reader behavior to be `Faithful` or an explicitly documented `Temporary adapter`.
...
| Adaptive EPUB page composition | On large portrait surfaces, a narrow Foliate text column leaves excessive blank paper even when the current chapter has more text available. Current emulator evidence: renderer `1232x1974`, paginated iframe `720x1440`, body `666px` wide. | The EPUB compositor must make margins feel natural by using available viewport capacity before leaving blank paper. For text-only content in the same chapter/section, more text should be shown through a wider page box or a controlled same-section multi-column/spread composition. Empty paper is acceptable at semantic boundaries only: explicit page break, end of chapter/section, cover/title page, image page with intrinsic limits, or user-selected margins. |
...
1. Fix the viewer/controller model: committed page sequence, chapter-local progress rail, hyperlink relocation state, native cover vs synthetic cover decision, single page-number owner, and adaptive EPUB page composition that uses available viewport capacity.
...
## 2026-06-17 Komikku SettingsItems Primitive Port Slice

Reference source:

- Komikku `tmp/references/komikku/presentation-core/src/main/java/tachiyomi/presentation/core/components/SettingsItems.kt`.
- Komikku `tmp/references/komikku/presentation-core/src/main/java/tachiyomi/presentation/core/components/Pill.kt`.
- Komikku `tmp/references/komikku/presentation-core/src/main/java/tachiyomi/presentation/core/components/material/Slider.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/settings/ReadingModePage.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/settings/GeneralSettingsPage.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/settings/ColorFilterPage.kt`.

Reference behavior:

- Reader settings controls are built from the shared `SettingsItems` primitives: `SettingsItemsPaddings`, `HeadingItem`, `SettingsChipRow`, `CheckboxItem`, `SliderItem`, `BaseSliderItem`, and `BaseSettingsItem`.
- Slider values use the shared pill/slider shape. The integer slider wraps the material slider by rounding the float value in `onValueChange`.
- The row primitive owns click behavior for checkboxes; callers provide an `onClick` toggle rather than maintaining Navic-only captured value rows.

Non-faithful Navic behavior found:

- The first settings component slice still left private Navic-local wrappers named `KomikkuSettingsChipRow`, `KomikkuSettingsCheckboxItem`, `KomikkuSettingsSliderItem`, and `KomikkuSettingsValuePill`.
- Those wrappers worked mechanically but were still a fork of Komikku's component boundary. Under the hard reference gate, working local wrappers are `Failing`, not complete.
- The PDF/Image guard still expected the old captured `cropBorders` assignment, which conflicted with Komikku's click-owned checkbox item shape.

Red guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderSettingsControlsUsePortedKomikkuSettingsItemsContract"
```

Result:

- Failed as expected at `ReaderRuntimeCommonChromeTest.kt:957` while production still lacked the ported `SettingsItems` primitive names and still had the Navic-local `KomikkuSettings*` wrappers.

Change:

- Replaced the local `KomikkuSettings*` wrappers with `SettingsItems`-shaped primitives in `ReaderSettingsDialog.kt`.
- Added local ports of `SettingsItemsPaddings`, `HeadingItem`, `SettingsChipRow`, `CheckboxItem`, `SliderItem`, `BaseSliderItem`, `BaseSettingsItem`, `Pill`, and `KomikkuIntegerSlider`.
- Updated checkbox call sites to pass click toggles through `CheckboxItem(...)`.
- Updated slider call sites to pass labels and absolute integer values through `SliderItem(...)`.
- Updated the stale PDF/Image acceptance guard to expect `pdfCropBorders = settings.pdfCropBorders != true`, matching the click-owned checkbox primitive.
- A compile check caught the `KomikkuIntegerSlider` local `onChange` function being declared after its use; the function was moved before the material `Slider(...)` call.
...
- Passed after the primitive port and `KomikkuIntegerSlider` ordering fix.
...
- First broader run failed on the stale PDF/Image checkbox assertion.
- Passed after the guard was reconciled with the ported checkbox contract.

Status:

- `Faithful` for the settings item primitive ownership shape and the removal of local `KomikkuSettings*` wrappers.
- `Failing` still remains for overall settings-dialog visual parity: surface palette, typography density, option grouping, phone/tablet screenshots, and direct comparison against Komikku still need follow-up.
- No APK or GitHub release was created for this slice.
...
## 2026-06-17 Komikku Chapter Navigator Tablet Padding Slice

Reference sources:

- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/components/ChapterNavigator.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/util/WindowSize.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/tachiyomi/util/system/DisplayExtensions.kt`.

Non-faithful Navic behavior found:

- Navic's `ReaderChapterNavigator.kt` still used hard-coded 8.dp outer padding for horizontal and vertical chapter navigators.
- Komikku uses `isTabletUi()` and switches the chapter navigator's outer horizontal/vertical padding from 8.dp to 24.dp on tablet UI.
- Leaving the rail at phone padding on tablets/foldables makes the side navigator proportions drift from the reference even when placement and slider ownership are correct.

Red guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderChapterNavigatorUsesKomikkuTabletPaddingContract"
```

Result:

- Failed as expected on `ReaderRuntimeCommonChromeTest.kt:525` while the platform tablet helper and adaptive padding were absent.

Change:

- Added `komikkuReaderIsTabletUi()` as a common expect helper in `ReaderPlatformHosts.kt`.
- Added Android actual `KomikkuReaderTabletUi.android.kt` using Komikku's `smallestScreenWidthDp >= 720` rule.
- Added iOS actual `KomikkuReaderTabletUi.ios.kt` as an explicit compile-preserving fallback; this is not a Komikku parity claim.
- Updated `ReaderChapterNavigator.kt` to use `horizontalPadding = if (isTabletUi) 24.dp else 8.dp` and `verticalPadding = if (isTabletUi) 24.dp else 8.dp`, matching Komikku's chapter navigator contract.
...
- First green attempt failed because the guard also rejected valid inner Komikku slider 8.dp padding.
- Guard was tightened to reject only the old top-level phone-only navigator padding.
- Second green attempt passed.

Status:

- `Faithful` for Android chapter navigator tablet-padding behavior against Komikku's 720dp smallest-width rule.
- `Temporary adapter` for iOS tablet detection until a platform-specific reader reference is defined.
- `Failing` still remains for broader progress-rail visual parity: button shape, slider colors, screenshot comparison, deterministic chapter/page semantics, and texture/page transition coupling are not completed by this slice.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Komikku Shared Integer Slider Primitive Slice
...
- Komikku exposes a reusable integer `Slider(...)` wrapper that accepts `value: Int`, `valueRange: IntProgression`, `SliderDefaults.Thumb`, `SliderDefaults.Track`, and rounds the Material3 float in `onValueChange = { onValueChange(it.roundToInt()) }`.
- `ChapterNavigator` and settings slider rows both route through that integer slider boundary instead of each owning their own direct Material3 float slider conversion.

Non-faithful Navic behavior found:

- `ReaderChapterNavigator.kt` imported `androidx.compose.material3.Slider` directly and locally converted float slider values with `roundToInt()`.
- `ReaderSettingsDialog.kt` had a private settings-only `KomikkuIntegerSlider(...)` wrapper with a narrower signature.
- Both implementations worked mechanically, but they were two Navic-owned copies of a Komikku shared primitive, so they were `Failing` under the no-working-exception gate.

Red guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderProgressAndSettingsUseSharedKomikkuIntegerSliderPrimitive"
```

Result:

- Failed as expected at `ReaderRuntimeCommonChromeTest.kt:510` while the shared primitive file was missing and the navigator still imported Material3 `Slider` directly.

Change:

- Added `KomikkuIntegerSlider.kt` as the shared reader-local port of Komikku's integer material slider primitive.
- Updated `ReaderChapterNavigator.kt` so `KomikkuChapterProgressSlider(...)` delegates integer value handling to `KomikkuIntegerSlider(...)`.
- Removed the private `KomikkuIntegerSlider(...)` duplicate from `ReaderSettingsDialog.kt`; settings slider rows now use the shared primitive.
- Updated older settings guards so they reject private settings-only slider wrappers instead of requiring them.
...
- Passed after the shared primitive extraction.

Status:

- `Faithful` for integer slider ownership shape shared by chapter navigator and settings rows.
- `Failing` still remains for broader progress-rail visual parity: screenshot comparison, exact tablet/foldable proportions, deterministic chapter/page semantics, and texture/page transition coupling are not completed by this slice.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Working-But-Non-Faithful Guard Tightening

Procedure change:

- The hard reference-parity gate now explicitly says: if a feature is not faithful, it must be redesigned even if it is supposed to be working.
- The guard applies to new features, bugfixes, and already-shipped reader behavior.
- A working implementation can only avoid immediate redesign if it is explicitly recorded as a `Temporary adapter` with the missing reference behavior, removal path, and user-accepted reason.
- This is a hard acceptance rule, not a wording preference. If a feature is only working because of a Navic-specific workaround, a Komikku-looking approximation, or an Anx/Foliate bypass, the feature status is `Failing` until it is redesigned around the reference product contract.

Verification:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderDevEnvironmentContractTest.komikkuReaderPlanRequiresReferenceParityBeforeReaderWork"
```

Result:

- Passed after the spec and `ReaderDevEnvironmentContractTest` were tightened to preserve the no-working-exception rule.

## 2026-06-17 Current Harness Revalidation

Scope:

- Local browser/reader harness only. This does not replace Android device validation and does not justify a GitHub release by itself.
- The goal was to separate current reproducible failures from old failures already recorded in this document.
...
- Output: `tools\reader-harness\output\epub-full-traversal.trace.json`.
- Stable full traversal reported `pageCount = 392` with progress checkpoints through `1/392`, `51/392`, `101/392`, `151/392`, `201/392`, `251/392`, `301/392`, and `351/392`.
- This supersedes the older local harness failure rows for unstable full-traversal denominator drift as the current local baseline. Keep those older rows as history, but do not keep treating them as current unless the failure reproduces again.
...
- Passed in the current worktree.
- Output: `tools\reader-harness\output\epub-native-drag-preview-underlay.json`.
- Evidence: the preview layer and iframe were present at a loaded section boundary with `targetIndex = 1`, `direction = next`, `side = right`, `width = 180px`, and themed background `rgb(243, 234, 215)`.

Status:

- `Faithful` is not claimed for the full drag/page-turn experience. The harness proves only that the adjacent-section underlay exists for one boundary drag case.
- `Failing` remains for broader texture/page-motion parity because the user still reported texture direction/intensity artifacts during real dragging.
- `Failing` remains for broader progress-rail visual parity and settings visual parity until screenshot/device comparisons are made against Komikku and the differences are redesigned, not patched around.
- No APK or GitHub release was created for this revalidation.

## 2026-06-17 Texture Key Stability Across Page-Profile Relabels

Reference requirement:

- Paper texture identity belongs to the rendered reader page. It must not change just because the pagination profile replaces a provisional total page count with the final deterministic count.
- Under the no-working-exception rule, a texture implementation that visually appears to work but relabels the same rendered page to a different texture key is `Failing`; it creates the side-flash/intensity-change class of artifact reported during page changes.

Failure reproduced:

- The current `epub-texture-page-turns` trace showed the same stable visible page identity, `OEBPS/Text/1.html|0`, being assigned two different texture keys as the page total changed:
  - `page:0:409`
  - `page:0:392`
- That means a single rendered page could swap paper variants after the deterministic page model finished sampling.
...
- Failed as expected after adding `assertTextureKeysIgnorePageCountOnlyRelabels(...)`.
- Error evidence: `Expected same-page texture key to ignore changing page totals; OEBPS/Text/1.html|0 changed ... page:0:409 -> ... page:0:392`.

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimePaperSurfaceTest.androidReaderPaperTextureVariesByRenderedPageLocatorInsideSameEpubSection"
```

Result:

- Failed as expected while `readerPaperTexturePageLocator(...)` still included `detail?.pageCount` in the texture locator.

Change:

- `readerPaperTexturePageLocator(...)` now uses the rendered `pageIndex` as `page:<index>` and does not include the total page count.
- The texture key still varies by stable page locator, section, and publication identity, but no longer changes when the denominator is relabeled from provisional to final.
- The harness assertion now runs in both `epub-texture-page-turns` and `epub-texture-frontmatter-transition` so this class of regression is checked in normal page turns and frontmatter/spine transitions.
...
- `Faithful` is not claimed for the whole texture transition system.
- This slice corrects one concrete texture instability: same rendered page, same section, different denominator no longer swaps the paper variant.
- `Failing` remains for texture motion parity until real drag transitions match Komikku-style page movement and Android screenshots/logs prove there is no side-flash/intensity artifact.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Chapter Rail Rejects Whole-Book Fallbacks

Reference requirement:

- Komikku's `ChapterNavigator` is chapter-scoped: the side rail displays and seeks inside the current chapter, while the reader/footer page model can remain whole-book.
- Under the no-working-exception rule, a rail that appears functional but receives `pageIndex/pageCount` from a whole-book EPUB model is `Failing`; it produces states like a later chapter showing `6 / 513` or `2 / 1748`.

Failure reproduced:

- Added a controller test with the device-shaped bad locator: `href = chapter-14.xhtml`, `pageIndex = 1`, `pageCount = 1748`, and no `chapterPageIndex/chapterPageCount`.
- Added a WebView source guard proving `chapterPagePosition(...)` backfilled missing reflowable section page math from the whole-book `pagePosition`.

Red guards:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderControllerTest.engineRelocationDoesNotFeedKomikkuRailFromGlobalPageModel" --tests "paige.navic.reader.ReaderRuntimeShellProgressTest.androidReaderDoesNotBackfillChapterProgressFromWholeBookPageModel"
```

Result:

- Failed as expected.
- `ReaderControllerTest > engineRelocationDoesNotFeedKomikkuRailFromGlobalPageModel FAILED`.
- `ReaderRuntimeShellProgressTest > androidReaderDoesNotBackfillChapterProgressFromWholeBookPageModel FAILED`.

Change:

- `ReaderChapterProgressState.updatedFrom(...)` now consumes only `chapterPageIndex`, `chapterPageCount`, and `chapterProgress` for the Komikku rail state. It no longer silently backfills chapter progress from global `locator.pageIndex/pageCount`.
- `chapterPagePosition(...)` now allows `pagePosition || fallback` only for fixed-layout publications. Reflowable EPUB chapter progress must come from `reflowableSectionPagePosition()`; if that is unavailable, the bridge omits chapter-local fields instead of sending global fields as chapter fields.
- This keeps the whole-book organic footer path separate from the Komikku chapter rail path.
...
```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderControllerTest" --tests "paige.navic.reader.ReaderRuntimeShellProgressTest"
```
...
- Passed.
- Stable traversal checkpoints stayed at `1/392`, `51/392`, `101/392`, `151/392`, `201/392`, `251/392`, `301/392`, and `351/392`.
...
- `Faithful` is not claimed for the whole progress rail visual design.
- This slice corrects the model boundary: reflowable EPUB chapter rail state no longer degrades into whole-book page counts when chapter-local data is temporarily unavailable.
- `Failing` remains for visual parity: rail height, colors, button size/placement, and screenshot comparison against Komikku still need redesign/validation.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Reader Bar Background Ownership Slice

Reference source:

- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/appbars/ReaderAppBars.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/appbars/ReaderTopBar.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/appbars/ReaderBottomBar.kt`.

Reference behavior:

- `ReaderAppBars` computes one shared reader bar `backgroundColor` using `MaterialTheme.colorScheme.surfaceColorAtElevation(3.dp)` and dark/light alpha: `0.9f` for dark theme, `0.95f` for light theme.
- The top and bottom bar contents are mounted inside that host-owned background. They do not create their own local `Surface` backgrounds or fixed Navic alpha values.
- The bottom bar receives navigation-bar insets from the app-bars host path, matching Komikku's bottom chrome ownership.

Non-faithful Navic behavior found:

- `KomikkuReaderTopBar` and `KomikkuReaderBottomBar` each created their own `Surface(...)`.
- Both private bar components used a fixed `.copy(alpha = 0.92f)`, diverging from Komikku's theme-aware bar alpha.
- The bottom bar did not receive `WindowInsets.navigationBars` in the host path.
- This worked mechanically, but under the no-working-exception rule it was still `Failing`: the bar surface/background owner was different from Komikku's app-bars layer.

Red guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderAppBarsOwnSharedKomikkuBarBackgroundAtHostLevel"
```

Result:

- Failed as expected at `ReaderRuntimeCommonChromeTest.kt:685` while `ReaderAppBars.kt` still lacked the host-level Komikku background imports/state and the private top/bottom bars still owned local `Surface` backgrounds.

Change:

- `KomikkuReaderAppBars(...)` now computes the shared Komikku bar background once using `isSystemInDarkTheme()`.
- Top chrome receives `.background(backgroundColor)` from the app-bars host and no longer creates a private `Surface`.
- Bottom chrome receives `.background(backgroundColor)`, padding, and `.windowInsetsPadding(WindowInsets.navigationBars)` from the app-bars host.
- `KomikkuReaderTopBar(...)` and `KomikkuReaderBottomBar(...)` now render transparent content over the host-owned reader bar background.
- The older bottom-action guard was tightened to inspect the whole bottom bar function body instead of stopping at `pointerInput(Unit) {}`.

Focused green guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderAppBarsOwnSharedKomikkuBarBackgroundAtHostLevel"
```

Result:

- Passed after the bar background ownership port.

Broader green guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest"
```

Result:

- First broader run failed on the stale bottom-action row substring guard.
- Passed after the guard was updated to inspect the whole bottom bar body.

Status:

- `Faithful` for reader bar background ownership shape against Komikku's app-bars host contract.
- `Failing` remains for full top/bottom chrome visual parity: exact `AppBar` typography/spacing, icon set, screenshot comparison, and tablet/foldable visual validation still need reference-backed follow-up.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Reader TopBar AppBar Primitive Slice

Reference source:

- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/appbars/ReaderTopBar.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/components/AppBar.kt`.

Reference behavior:

- `ReaderTopBar` delegates to the reusable `AppBar(...)` primitive with `backgroundColor = Color.Transparent`.
- `AppBar(...)` delegates title rendering to `AppBarTitle(...)` and actions to `AppBarActions(...)`.
- Bookmark state is represented as an `AppBar.Action` using outlined bookmark icons, not a custom reader-only row.

Non-faithful Navic behavior found:

- `KomikkuReaderTopBar(...)` was still a Navic-local `Row` with fixed horizontal/vertical padding, `headlineSmall` title, `bodyLarge` subtitle, and manually spaced actions.
- That custom row was visually serviceable, but it preserved a Navic-specific top-bar implementation instead of Komikku's reusable AppBar/AppBarActions path.
- Under the no-working-exception rule, the custom row was `Failing` even if it appeared to work.

Red guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderTopBarUsesPortedKomikkuAppBarPrimitive"
```

Result:

- Failed as expected at `ReaderRuntimeCommonChromeTest.kt:731` while production still lacked `KomikkuReaderAppBar(...)`, `KomikkuReaderAppBarTitle(...)`, `KomikkuReaderAppBarActions(...)`, and `KomikkuReaderAppBarAction`.

Change:

- Added a reader-local port of Komikku's AppBar primitive shape in `ReaderAppBars.kt`:
  - `KomikkuReaderAppBar(...)`
  - `KomikkuReaderAppBarTitle(...)`
  - `KomikkuReaderAppBarActions(...)`
  - `KomikkuReaderAppBarAction.Action`
- `KomikkuReaderTopBar(...)` now delegates to `KomikkuReaderAppBar(...)` with `backgroundColor = Color.Transparent`, title/subtitle inputs, back navigation, and bookmark action state.
- The subtitle now uses the AppBar-title pattern with `bodyMedium`, ellipsis, and `basicMarquee(...)` rather than Navic's earlier fixed `bodyLarge` row layout.

Focused green guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderTopBarUsesPortedKomikkuAppBarPrimitive"
```

Result:

- Passed after the AppBar primitive port.
...
- `Faithful` for the top-bar primitive ownership shape against Komikku's `ReaderTopBar` / `AppBar` / `AppBarActions` reference.
- `Failing` remains for full top-bar visual parity until screenshots are compared against Komikku on phone/foldable/tablet and any remaining spacing/icon/typography deviations are redesigned from the reference.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Reader BottomBar Action Model Slice

Reference source:

- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/appbars/ReaderBottomBar.kt`.
- Google Material Icons repository SVGs:
  - `https://raw.githubusercontent.com/google/material-design-icons/master/src/editor/format_list_numbered/materialiconsoutlined/24px.svg`
  - `https://raw.githubusercontent.com/google/material-design-icons/master/src/action/settings/materialiconsoutlined/24px.svg`

Reference behavior:

- `ReaderBottomBar(...)` renders centered, evenly distributed actions through reference bottom-button semantics.
- The chapter-list action uses `Icons.Outlined.FormatListNumbered`.
- The settings action uses `Icons.Outlined.Settings`.
- Bottom actions are not a Navic-only hard-coded toolbar using `Icons.Outlined.List` and `Icons.Filled.Settings`.

Non-faithful Navic behavior found:

- `KomikkuReaderBottomBar(...)` still hard-coded three icon buttons directly inside the row.
- Contents used `Icons.Outlined.List` instead of Komikku's `FormatListNumbered` chapter-list affordance.
- Settings used the filled settings icon even though the Komikku reader bottom bar uses outlined settings.
- This worked mechanically, but under the no-working-exception rule it was still `Failing`: the action model and visible icons diverged from the reference product.

Red guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel --rerun-tasks "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderBottomBarUsesKomikkuBottomButtonActionModel"
```

Result:

- Failed as expected at `ReaderRuntimeCommonChromeTest.kt:784` while production still lacked `KomikkuReaderBottomAction`, `FormatListNumbered`, and outlined settings in the bottom bar.

Change:

- Added the missing Material outlined SVGs to the Valkyrie icon source set:
  - `composeApp/src/commonMain/valkyrieResources/outlined/ic_format_list_numbered.svg`
  - `composeApp/src/commonMain/valkyrieResources/outlined/ic_settings.svg`
- `KomikkuReaderAppBars(...)` now builds `bottomActions = listOf(...)` for the reader bottom bar.
- `KomikkuReaderBottomBar(...)` now accepts `actions: List<KomikkuReaderBottomAction>` and renders actions through a `when (action)` mapping.
- Added local `KomikkuReaderBottomAction` sealed interface with `ViewChapters`, `ReadingMode`, and `Settings`.
- Contents now renders `Icons.Outlined.FormatListNumbered`.
- Settings now renders `Icons.Outlined.Settings`.

Focused green guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderBottomBarUsesKomikkuBottomButtonActionModel"
```

Result:

- Passed after the bottom action model and icon sources were added.
- Valkyrie regenerated 67 outlined icons, confirming the two new SVG sources were consumed.
...
- Passed after the bottom action model port.

Status:

- `Faithful` for the bottom bar action model and icon choices currently implemented in Navic against Komikku's `ReaderBottomBar` reference.
- `Failing` remains for full bottom-bar parity: the enabled-button preference set, webview/browser/share/rotation/crop/page-layout/shift actions, resource string localization, exact spacing/opacity, and screenshot comparison still need follow-up.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Non-Faithful Working Feature Blocker

User direction:

- "If a feature is not faithful is should be redesigned even if it is supposed to be working."

Procedure change:

- Added `Non-Faithful Working Feature Blocker` under the hard reference-parity gate.
- Working status is now explicitly diagnostic evidence, not acceptance evidence.
- A non-faithful feature cannot be used as the foundation for polish, dependent UI, settings, release notes, or new engine behavior.
- No release candidate may be created from a slice whose touched reader behavior is still `Failing`.
- Release candidates require every touched reader behavior to be `Faithful` or an explicitly documented `Temporary adapter`.

Guard:

- `ReaderDevEnvironmentContractTest.komikkuReaderPlanRequiresReferenceParityBeforeReaderWork` now requires the blocker wording so the rule survives compaction and future refactors.

Status:

- This is a project procedure gate, not a reader feature implementation.
- No APK or GitHub release was created for this documentation/test guard update.

## 2026-06-17 Reader BottomButton Enabled-Set Model Slice

Reference source:

- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/presentation/reader/appbars/ReaderBottomBar.kt`.
- Komikku `tmp/references/komikku/app/src/main/java/eu/kanade/tachiyomi/ui/reader/setting/ReaderBottomButton.kt`.

Reference behavior:

- `ReaderBottomBar(...)` receives an enabled-button set and checks optional bottom actions with `ReaderBottomButton.<Action>.isIn(enabledButtons)`.
- `ReaderBottomButton` owns stable string values such as `vc`, `wb`, `rm`, `cbp`, and `pl`.
- `ReaderBottomButton.BUTTONS_DEFAULTS` records Komikku's default optional buttons.
- Settings is not part of the optional enabled-button set. It is always rendered as the final bottom action.

Non-faithful Navic behavior found:

- The previous Navic slice had a `KomikkuReaderBottomAction` sealed interface and a fixed `bottomActions = listOf(ViewChapters, ReadingMode, Settings)`.
- That improved the icon mapping, but it still diverged from Komikku's enabled-button registry. It made settings look like a configurable action even though Komikku renders settings unconditionally.
- Under the no-working-exception rule, the fixed action list was `Failing` even though the buttons worked.
...
- Failed as expected at `ReaderRuntimeCommonChromeTest.kt:793` while production still lacked `KomikkuReaderBottomButton` and enabled-set gating.

Change:

- Replaced `KomikkuReaderBottomAction` with `KomikkuReaderBottomButton(val value: String)`.
- Ported the Komikku bottom-button values: `ViewChapters`, `WebView`, `Browser`, `Share`, `ReadingMode`, `Rotation`, `CropBordersPager`, `CropBordersContinuesVertical`, `CropBordersWebtoon`, and `PageLayout`.
- Added `fun isIn(buttons: Collection<String>) = value in buttons`.
- Added `BUTTONS_DEFAULTS` with Komikku's reference optional defaults.
- Changed `KomikkuReaderBottomBar(...)` to receive `enabledButtons: Set<String>`.
- `ViewChapters` and `ReadingMode` now render through enabled-button checks.
- Settings now renders unconditionally as the final bottom action, matching Komikku's bottom bar ownership.
...
- Passed after the enum and enabled-set model were added.
...
- Passed after the bottom-button enabled-set port.

Status:

- `Faithful` for the bottom-button registry shape, optional-action enabled-set check, and unconditional settings action.
- `Temporary adapter`: Navic currently feeds `NAVIC_SUPPORTED_DEFAULTS` instead of a user-editable Komikku bottom-button preference because WebView/browser/share/rotation/crop/page-layout callbacks are not all controller-backed yet. Removal path: add a reader bottom-button preference surface, route each optional action through controller callbacks, then feed the persisted enabled set into `KomikkuReaderBottomBar(...)`.
- `Failing` remains for full bottom-bar parity: user-editable enabled buttons, optional WebView/browser/share/rotation/crop/page-layout/shift actions, resource string localization, exact spacing/opacity, and screenshot comparison still need follow-up.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Reader BottomBar Resource Labels Slice
...
- Komikku bottom-bar buttons use resource-backed content descriptions, not hardcoded labels: `stringResource(MR.strings.chapters)`, `stringResource(MR.strings.viewer)`, and `stringResource(MR.strings.action_settings)`.
- Labels are part of the same action model as the icons. A button that works but has hardcoded or non-reference label ownership is still not faithful.

Non-faithful Navic behavior found:

- The earlier slice still used hardcoded `contentDescription = "Contents"`, `"Reading mode"`, and `"Settings"` in `ReaderAppBars.kt`.
- Under the no-working-exception rule, these were `Failing` even though the buttons were clickable.
...
- Failed as expected while the common chrome test required resource-backed labels and production still used hardcoded content descriptions.

Change:

- Added `action_reader_reading_mode` to Compose resources.
- Changed the bottom-bar content descriptions to `stringResource(Res.string.title_chapters)`, `stringResource(Res.string.action_reader_reading_mode)`, and `stringResource(Res.string.title_settings)`.
- Added the generated resource-key imports required by Compose Multiplatform resource accessors.
...
- `build\codex\bottom-button-labels-green-2.exit` was `0`.
...
- `build\codex\common-chrome-after-bottom-button-labels.exit` was `0`.
- `BUILD SUCCESSFUL in 9s`.

Status:

- `Faithful` for bottom-bar content-description ownership: labels are now resource-backed like Komikku rather than hardcoded in the reader chrome.
- `Temporary adapter`: Navic uses its existing `title_chapters` and `title_settings` string keys plus a new `action_reader_reading_mode` key, because Navic does not share Komikku's exact resource catalog. Removal path: when the full bottom-button preference/settings surface is ported, verify every bottom-button label against the Komikku source and align naming/locale coverage as needed.
- `Failing` remains for full bottom-bar parity: user-editable enabled buttons, optional WebView/browser/share/rotation/crop/page-layout/shift actions, exact spacing/opacity, screenshot comparison, and device validation still need follow-up.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Chapter Navigator Cover And Short-Section Policy Slice
...
- Komikku's `ChapterNavigator` owns previous/next chapter buttons, page text, slider, haptic drag feedback, RTL action inversion, and resource-backed previous/next content descriptions.
- Komikku hides the slider when `totalPages <= 1` by replacing the slider slot with `Spacer(Modifier.weight(1f))`.

User product policy:

- The reader should not show the progress bar over the shell cover because the cover is a single virtual page.
- Very short sections should not render a full progress slider. Navic uses `totalPages >= 3` as the threshold for showing the slider/track.

Reference gap:

- The `totalPages >= 3` threshold is intentionally stricter than Komikku's `totalPages > 1`. This is a documented Navic reader policy on top of the Komikku navigator structure, not a claim of pure Komikku parity.

Non-faithful Navic behavior found:

- `ReaderAppBars.kt` still mounted `KomikkuChapterNavigator(...)` while `controllerState.shellCoverVisible` was true.
- `ReaderChapterNavigator.kt` rendered the slider at `totalPages > 1`, which made two-page sections feel over-controlled.
- Previous/next chapter labels were hardcoded as `"Previous"` and `"Next"` instead of using resource-backed labels like Komikku.
...
- `build\codex\chapter-navigator-labels-red.exit` was `1`; the first red pass failed at the new resource-label assertions.
- `build\codex\chapter-navigator-cover-short-red.exit` was `1`; the second red pass failed at the cover/short-section and resource-label assertions.

Change:

- Added `action_previous_chapter` and `action_next_chapter` Compose string resources.
- Changed `ReaderChapterNavigator.kt` previous/next button content descriptions to `stringResource(Res.string.action_previous_chapter)` / `stringResource(Res.string.action_next_chapter)`, including the same RTL label inversion shape used by Komikku.
- Added `readerShouldShowChapterProgressSlider(totalPages) = totalPages >= 3` and routed both horizontal and vertical navigator slider slots through it.
- Added `showChapterNavigator = visible && !controllerState.shellCoverVisible` in `ReaderAppBars.kt`, so the chapter navigator is not mounted while the native shell cover is visible.
- Bottom-nav mode now also skips `KomikkuChapterNavigator(...)` while `controllerState.shellCoverVisible` is true.

Focused green guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderChromeUsesKomikkuEquivalentSideProgressRail" --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderVerticalProgressRailUsesKomikkuSliderOwnedNavigator" --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderChapterNavigatorHonorsRtlDirectionLikeKomikku"
```

Result:

- `build\codex\chapter-navigator-cover-short-green-2.exit` was `0`.
...
- `build\codex\common-chrome-after-chapter-cover-short.exit` was `0`.
- `BUILD SUCCESSFUL in 11s`.

Status:

- `Faithful` for the chapter navigator's resource-backed previous/next labels and RTL label/action mapping against Komikku.
- `Temporary adapter`: Navic hides the progress slider until `totalPages >= 3`, which intentionally diverges from Komikku's `totalPages > 1`. Removal path: keep this as a named reader product policy if device testing confirms it improves short-section behavior; otherwise revert the threshold to Komikku's exact `> 1` condition.
- `Faithful` for not showing the chapter navigator while the shell cover is visible, because the shell cover is a Navic virtual page outside Komikku's manga chapter model and should not expose a chapter progress control.
- `Failing` remains for full progress-rail visual parity: exact rail proportions, thumb/track color, screenshot comparison, deterministic chapter-page semantics, and texture/page transition coupling are not completed by this slice.
- No APK or GitHub release was created for this slice.

## 2026-06-17 Local Reader Harness Revalidation

Scope:

- Revalidate the current browser-path reader runtime after the Komikku chapter-navigator and deterministic pagination-profile slices.
- This is not Android-device acceptance. It only proves the local Chromium/WebView-like harness path is internally stable before the next APK candidate.
...
- Passed.
- Output: `tools\reader-harness\output\epub-texture-page-turns.trace.json`.
...
- Passed.
- Output: `tools\reader-harness\output\epub-texture-frontmatter-transition.trace.json`.
...
- Passed.
- Output: `tools\reader-harness\output\epub-pagination-profile.trace.json`.
...
- Passed.
- Output: `tools\reader-harness\output\epub-full-traversal.trace.json`.
- Stable full traversal reported `pageCount = 392` with checkpoints `1/392`, `51/392`, `101/392`, `151/392`, `201/392`, `251/392`, `301/392`, and `351/392`.

Interpretation:

- The local browser-path pagination profile is currently stable for the Hobbit fixture at the Android-like `500x960 @ 3x` viewport.
- The local browser-path texture movement guards currently pass for normal page turns and the frontmatter transition.
- The local browser-path drag preview underlay guard currently passes.
- Any remaining progress-rail, gesture, texture, or cover behavior observed on device should be treated as Android/native-shell integration evidence, not as proof that the browser pagination profile is still drifting.
- Next APK/device validation should still cover the shell cover, chapter rail hidden-on-cover behavior, chapters with fewer than three pages, Chapter 13/14/15 transitions, image long-press vs drag, hyperlink navigation, and PDF navigation.

## 2026-06-17 PDF Fixed-Layout Duplicate Tap Coalescing Slice

Reference authority:

- Anx/Foliate remains the PDF engine authority: `tmp/references/anx-reader/assets/foliate-js/src/pdf.js` and Navic's bundled `composeApp/src/androidMain/assets/reader/vendor/foliate-js/pdf.js` expose PDF pages as fixed-layout Foliate sections.
- Komikku remains the shell/input authority: reader gestures should not create accidental multiple page turns from one perceived tap burst.

User-observed symptom:

- PDF navigation felt sluggish and sometimes jumped several pages at once when using tap actions.

Local red guard:

- The repo had no committed PDF fixture, so a temporary five-page fixture was generated at `tmp\reader-harness\pdf-navigation-fixture.pdf`.

```powershell
node tools\reader-harness\src\run-reader-harness.mjs --mode pdf-smoke --fixture tmp\reader-harness\pdf-navigation-fixture.pdf --viewport-width 500 --viewport-height 960 --device-scale-factor 3
```

Result before the fix:

- Failed with `Expected coalesced PDF double nextPage to advance only one page; observed 1 -> 3`.
- Trace showed two simultaneous `nextPage` dispatches. The second command entered `page-turn:queued` while the first fixed-layout `goTo(2)` was still in-flight, then immediately executed after settlement and advanced again to page index `3`.

Change:

- `navic-reader.js` now coalesces same-direction duplicate fixed-layout turns while `pageTurnPromise` is active.
- In that case it emits `page-turn:coalesced` and returns the active turn promise instead of pushing another queued turn.
- Opposite-direction in-flight turns and truly sequential awaited turns still use the existing page-turn path.

Green guards:

```powershell
node tools\reader-harness\src\run-reader-harness.mjs --mode pdf-smoke --fixture tmp\reader-harness\pdf-navigation-fixture.pdf --viewport-width 500 --viewport-height 960 --device-scale-factor 3
node tools\reader-harness\src\run-reader-harness.mjs --mode pdf-fast-sequential-turns --fixture tmp\reader-harness\pdf-navigation-fixture.pdf --viewport-width 500 --viewport-height 960 --device-scale-factor 3
node tools\reader-harness\src\run-reader-harness.mjs --mode pdf-image-settings --fixture tmp\reader-harness\pdf-navigation-fixture.pdf --viewport-width 500 --viewport-height 960 --device-scale-factor 3
node --check composeApp\src\androidMain\assets\reader\navic-reader.js
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeNavigationFlowTest.androidReaderCoalescesDuplicateFixedLayoutPageTurnTapsBeforeQueueing"
```

Results:

- `pdf-smoke`: passed. The trace now includes `page-turn:coalesced`, and the double-next checkpoint lands at page index `2` instead of jumping to `3`.
- `pdf-fast-sequential-turns`: passed. Two awaited sequential turns still advance two pages.
- `pdf-image-settings`: passed. PDF fit/crop/gap settings still reach the fixed-layout renderer.
- `node --check`: passed.
- Focused Android host guard: passed.
- Broader `ReaderRuntimeNavigationFlowTest` host suite: passed after the coalescing fix.

Status:

- `Faithful` for the shell/controller rule that one perceived fixed-layout tap burst should not become multiple page turns.
- `Temporary adapter`: this still operates inside the Foliate fixed-layout bridge rather than a concrete Komikku-style native PDF viewer. Removal path: move fixed-layout PDF paging into a concrete viewer adapter while keeping the same coalescing semantics at the controller/viewer boundary.
- Android device validation is still required before claiming the user-facing PDF tap feel is fixed on the installed APK.

## 2026-06-17 Emulator Verification Procedure Regression

Why this section exists:

- A compaction/resume event caused the active work to drift back to manual ADB checks even though the repo already has dedicated reader-dev validation scripts.
- The emulator initially showed old behavior because it was running a stale `darkaxt.navic.readerdev` install from `2026-06-17 03:34:20`.
- The relevant source/assets had newer timestamps: `ReaderAppBars.kt` at `2026-06-17 11:23:48`, `ReaderChapterNavigator.kt` at `2026-06-17 11:28:48`, and `navic-reader.js` at `2026-06-17 12:28:55`.
- A fresh `:androidApp:assembleReaderDev` build was created and installed; `darkaxt.navic.readerdev` then reported `lastUpdateTime=2026-06-17 12:45:52`.

Required correction:

- Do not claim emulator verification from the stale-install checks or from the manual screenshot taken during this incident.
- The correct next step is to fix the scripted matrix path and rerun it, not to continue with manual ADB gestures.

Matrix script failure observed:

```powershell
.\scripts\adb-reader-komikku-matrix.ps1 -Package 'darkaxt.navic.readerdev' -ApkPath 'androidApp\build\outputs\apk\readerDev\Navic.apk' -ExpectedVersionName 'v1.0.11-eta69' -ArtifactRoot 'captures\reader-komikku-matrix\eta69-current-emulator-20260617-124831' -IncludeCoverChecks -ContinueOnFailure
```

Result:

- Failed before reaching real reader validation.
- The wrapper's array forwarding to `adb-reader-smoke.ps1` caused PowerShell to bind values positionally instead of by parameter name.
- Symptoms included `APK not found: ...\darkaxt.navic.readerdev` and tap fractions being bound to `TapPreset`.

Status:

- `Fixed locally` for the emulator matrix parameter-forwarding plumbing.
- Added a focused host guard in `ReaderKomikkuBackboneResetTest.adbKomikkuReaderMatrixRunsNamedNativeFrameChecks()` requiring named hashtable splatting into `adb-reader-smoke.ps1` and rejecting positional `@args` forwarding.
- `adb-reader-komikku-matrix.ps1` now forwards `Package`, `ApkPath`, tap/swipe/long-press fractions, validation switches, texture direction, and `DeviceSerial` by name through `$smokeArgs`.
- `adb-reader-smoke.ps1` now accepts `-DeviceSerial` and sets `ANDROID_SERIAL` so raw ADB calls inside the smoke script target the intended device/emulator.
- Focused green guard passed:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel '-Pkotlin.incremental=false' :composeApp:testAndroidHost --tests 'paige.navic.reader.ReaderRuntimeAssetsTest.adbReaderSmokeCapturesFocusedReaderDiagnostics' --tests 'paige.navic.reader.ReaderKomikkuBackboneResetTest.adbKomikkuReaderMatrixRunsNamedNativeFrameChecks'
```
...
- `Pending` for actual emulator product validation. The first retry after fixing named forwarding was invalid because both `RFCY80551LT` and `emulator-5554` were attached and the matrix did not yet force a target serial.
- The first serial-targeted matrix runs were also invalid as reader evidence because the matrix was run with `-ApkPath` and without `-NoLaunch`; that reinstalled and monkey-launched `darkaxt.navic.readerdev` into the Books library rather than the reader.
- Invalid artifact roots from this incident:
  - `captures\reader-komikku-matrix\eta69-current-emulator-serial-20260617-131511`: script stderr/progress handling still broken.
  - `captures\reader-komikku-matrix\eta69-current-emulator-serial-20260617-132220`: script stderr/progress handling still broken.
  - `captures\reader-komikku-matrix\eta69-current-emulator-serial-20260617-132532`: script empty-diagnostics handling still broken.
  - `captures\reader-komikku-matrix\eta69-current-emulator-serial-20260617-133039`: baseline passed install/version but UI hierarchy showed the Books library, not the reader.
  - `captures\reader-komikku-matrix\eta69-current-emulator-serial-20260617-133639`: script plumbing was clean, but baseline still showed the Books library, so all reader gesture failures from that run are invalid product evidence.
- Script guard fixes completed during this incident:
  - `adb-reader-smoke.ps1` captures native `adb` output internally and restores `$ErrorActionPreference`, so normal `adb pull` progress is not treated as a terminating error.
  - `Get-ReaderTextureDirectionSamples` accepts empty diagnostic input and returns a plain array so missing texture logs become validation failures, not PowerShell binding failures.
  - Focused guard passed:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel '-Pkotlin.incremental=false' :composeApp:testAndroidHost --tests 'paige.navic.reader.ReaderRuntimeAssetsTest.adbReaderSmokeCapturesFocusedReaderDiagnostics'
```

Result: `BUILD SUCCESSFUL`.

- The next required run must first open the reader through `install-reader-dev.ps1`, then run the matrix against the existing foreground reader state:
...
- Before any further Android reader behavior claim, rerun that matrix, inspect the baseline screenshot/window hierarchy to confirm it is the reader, inspect `reader-matrix-summary.csv`, `reader-matrix-failures.txt`, screenshots, and diagnostics, then append the artifact root plus pass/fail summary here.

## 2026-06-17 Emulator Launch Readiness Gate Hardening

Why this was needed:

- The previous emulator verification procedure still allowed a false-positive setup capture: `darkaxt.navic.readerdev` was foreground, but the screenshot could be taken while Navic was still on the splash screen or before the reader WebView had emitted a real ebook-ready signal.
- This is exactly the failure mode that made compaction recovery drift into manual ADB checks. Foreground is necessary, but not sufficient, for reader validation.

Changes:

- `scripts/install-reader-dev.ps1` now captures native `adb` stderr/progress as text and throws only on non-zero exit.
- The same script now includes `Wait-ReaderDevForeground`, using `adb shell dumpsys activity activities` and `mCurrentFocus` / `mFocusedApp` to prove the app is focused.
- The same script now includes `Wait-ReaderDevPublicationReady`, using `adb logcat -d -v brief` and the reader bridge `publicationReady` event to prove the ebook runtime has loaded before capture.
- When launching a direct reader target, the script clears logcat before `am start` so a stale `publicationReady` event cannot satisfy the wait.
- Redirected script logs now include the foreground/publication wait messages, so background validation artifacts are readable after compaction.

Red/green guards:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel '-Pkotlin.incremental=false' :composeApp:testAndroidHost --tests 'paige.navic.reader.ReaderDevEnvironmentContractTest.readerDevScriptsAndSecretFilesAreDocumentedAndIgnored'
```

- Red: `build\codex\reader-dev-install-publication-ready-red.out.log`, exit `1`, failed because `install-reader-dev.ps1` did not yet contain `Wait-ReaderDevPublicationReady`.
- Green: `build\codex\reader-dev-install-publication-ready-green.out.log`, exit `0`, `BUILD SUCCESSFUL in 11s`.
- Green after log/output cleanup: `build\codex\reader-dev-install-publication-ready-green-2.out.log`, exit `0`, `BUILD SUCCESSFUL in 8s`.

Emulator script validation:

```powershell
.\scripts\install-reader-dev.ps1 -EnvFile 'C:\Users\darka\Documents\Projects\Android\Navic\bindery-debug.env' -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -NoBuild -NoInstall -Capture
```

Result:

- Exit `0`.
- Transcript: `build\codex\install-reader-dev-publication-ready-20260617-141919.out.log`.
- Screenshot: `captures\reader-dev\reader-dev-20260617-141946.png`.
- Transcript includes:
  - `Waiting for darkaxt.navic.readerdev to become the focused Android window before capture...`
  - `Foreground confirmed: mFocusedApp=ActivityRecord{... darkaxt.navic.readerdev/paige.navic.androidApp.MainActivity ...}`
  - `Waiting for reader publicationReady bridge event before capture...`
  - `Reader publication ready: I/ReaderEngineWebViewHost(...): Reader bridge raw: {"type":"publicationReady"}`
- Screenshot shows the loaded ebook cover for `Alcatraz versus the Evil Librarians`, not the Android launcher, Books library, or Navic splash.

Status:

- `Fixed locally` for the dev-loop readiness gate and compaction-recovery procedure.
- `Not a product behavior claim`: this did not run the full Komikku matrix and does not claim tap/drag/progress/texture behavior is fixed.
- The next product validation still must run:
...
- Matrix runs are invalid unless `install-reader-dev.ps1` has first opened the reader and the matrix baseline screenshot/window hierarchy confirms the reader surface.

## 2026-06-17 eta69 Emulator Komikku Matrix After Readiness Gate

Command:

```powershell
$expectedVersionName = (Select-String -Path 'androidApp\build.gradle.kts' -Pattern 'versionName = "([^"]+)"').Matches.Groups[1].Value
.\scripts\adb-reader-komikku-matrix.ps1 -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -ExpectedVersionName $expectedVersionName -ArtifactRoot 'captures\reader-komikku-matrix\eta69-emulator-komikku-gate-20260617-142645' -NoLaunch -IncludeCoverChecks -ContinueOnFailure
```

Result:

- Valid reader baseline. `baseline-current-reader/screen.png` shows the loaded ebook cover, not launcher, login, Books grid, or splash.
- Installed package evidence: `versionName=v1.0.11-eta69`, `versionCode=402`, `lastUpdateTime=2026-06-17 14:06:44`.
- Matrix artifact root: `captures\reader-komikku-matrix\eta69-emulator-komikku-gate-20260617-142645`.
- Summary file: `reader-matrix-summary.csv`.
- Failure file: `reader-matrix-failures.txt`.
- Background wrapper note: the matrix exits the PowerShell host on failures, so the outer background exit-marker file was not written. The matrix artifact files were completed at `2026-06-17 14:28:06` and are the authoritative result for this run.
...
- `edge-tap-next`: native tap was recorded, but no moved texture samples were captured for direction `next`.
- `edge-tap-previous`: native left tap was recorded, but no moved texture samples were captured for direction `previous`.
- `drag-next`: diagnostics recorded `Reader shell cover drag candidate`, `Reader shell cover swipe`, and `Reader shell cover command`, but no native reader drag preview. This means the next-drag path is still being owned by shell-cover handling in this state.
- `texture-next-walk`: repeated taps were classified as `MENU`, not page-next, and no moved texture samples were captured.
- `texture-previous-walk`: repeated left taps were classified as `LEFT`, but no moved texture samples were captured.

Interpretation:

- The emulator procedure is now strong enough to expose real shell/controller failures.
- Reader loading, center menu tap, native long press separation, cover center tap, cover drag-next, and previous drag instrumentation are currently observable.
- Directional tap/drag/texture ownership is still `Failing`, not `Faithful`, because the current controller does not consistently produce Komikku-style page-zone actions and texture movement evidence.
- Next implementation slice should focus on native reader input ownership and texture movement diagnostics together. Do not treat isolated cover drag success or center tap success as a completed Komikku controller port.

## 2026-06-17 eta69 Emulator Matrix After Effective Display-Size Fix
...
- Exit `0`.
- Transcript: `build\codex\install-reader-dev-emulator-20260617-145344.out.log`.
- Screenshot: `captures\reader-dev\reader-dev-20260617-145412.png`.
- Transcript includes foreground confirmation and `Reader bridge raw: {"type":"publicationReady"}` before screenshot capture.
- Screenshot shows the loaded ebook cover for `Alcatraz versus the Evil Librarians`, not launcher, login, Books grid, or splash.

Matrix command:

```powershell
$expectedVersionName = (Select-String -Path 'androidApp\build.gradle.kts' -Pattern 'versionName = "([^"]+)"').Matches.Groups[1].Value
.\scripts\adb-reader-komikku-matrix.ps1 -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -ExpectedVersionName $expectedVersionName -ArtifactRoot 'captures\reader-komikku-matrix\eta69-emulator-after-size-fix-20260617-145510' -NoLaunch -IncludeCoverChecks -ContinueOnFailure
```
...
- `versionCode=402`
- `lastUpdateTime=2026-06-17 14:06:44`
- Effective emulator size: `Override size: 1848x2960`; the smoke script now uses that effective size for fraction gestures.
- Production reader source/assets checked for this run were older than the installed package. The only relevant uninstalled change was the validation script/display-size parser, so `-NoBuild -NoInstall` was valid.

Artifact evidence:

- Matrix artifact root: `captures\reader-komikku-matrix\eta69-emulator-after-size-fix-20260617-145510`.
- Summary file: `reader-matrix-summary.csv`.
- Failure file: `reader-matrix-failures.txt`.
- Background wrapper note: the matrix produced summary/failure files at `2026-06-17 14:56:26`, but the outer wrapper did not write its exit marker because the matrix throws from inside the PowerShell host when failures are present. The artifact files are the authoritative result.
...
- `edge-tap-next`: native tap classification was correct (`Reader native tap action=RIGHT x=1663.0 y=1480.0 width=1848 height=2960`), but no `nextPage` command or texture movement was captured.
- `edge-tap-previous`: native tap classification was correct (`Reader native tap action=LEFT x=185.00098 y=1480.0 width=1848 height=2960`), but no texture movement was captured.
- `drag-next`: diagnostics captured shell-cover ownership (`Reader shell cover drag candidate`, `Reader shell cover swipe`, and `Reader shell cover command`) rather than native readable drag preview.
- `cover-drag-next`: diagnostics captured native readable drag preview and texture movement, not shell-cover swipe. This means the matrix step was no longer running on the cover by the time the cover check executed.

Interpretation:

- The display-size parser fix is valid. The previous `texture-next-walk`/`texture-previous-walk` failures were caused by using the physical `1080x2400` size instead of the effective `1848x2960` override size. Both texture walk directions now pass and show non-inverted movement.
- The remaining matrix failures cannot all be treated as product failures. The current matrix starts from a shell cover, runs content edge/drag checks before explicitly entering readable content, and then runs cover checks after content navigation has already changed state.
- The next required slice is a validation-script fix, not a reader product fix: split cover checks from readable-content checks and add an explicit state transition/readiness step so content checks run on readable EPUB pages and cover checks run on the shell cover.
- Until that script ordering is fixed, do not publish a release or claim remaining edge/drag failures as product regressions from this artifact alone.

## 2026-06-17 Adaptive EPUB Page Composition Guard

User acceptance rule:

- EPUB margins should feel natural, but if there is more room for text, more text should be shown.
- Empty paper is acceptable only when the current content reaches a semantic boundary or intrinsic content limit: explicit page break, end of chapter/section, cover/title page, image page constraints, or user-selected margins.
- A narrow centered text box on a large portrait surface is not acceptable when the same chapter/section has additional text available.

Root cause evidence:

- The emulator page that triggered this slice rendered the Android/WebView surface at `1232x1974`, but Foliate's paginated iframe stayed near `720x1440` and the body stayed roughly `666px` wide.
- `foliate-js/paginator.js` defaults `--_max-inline-size: 720px` and `--_max-block-size: 1440px`. Preserving those defaults strands readable capacity as blank paper on foldable/tablet-sized surfaces.

Change:

- `readerAdaptiveFoliatePageBox(...)` computes the Foliate page box from the current viewport and reader margin setting.
- `applyReaderViewportLayout(...)` and `applyReaderViewportLayoutToProfilerView(...)` now set `max-inline-size`, `max-block-size`, and `max-column-count` on non-fixed-layout Foliate renderers.
- Pagination render metadata now includes the adaptive page box so deterministic page-count cache entries are tied to the layout policy that produced them.
- The current implementation enlarges the page box and enables two-column composition on wide surfaces. Portrait same-section page combining remains a future controller feature; this slice does not claim to solve that.
...
- Red: failed before the harness guard existed with `Unsupported reader harness mode: adaptive-page-box-logic`.
- Green: passed after adding the guard. The guard rejects the old `720x1440` Foliate page box at `1232x1974`, requires large portrait surfaces to use more than `1000px` inline and `1800px` block capacity, verifies user margin only reduces horizontal paper capacity, and verifies wide surfaces enable controlled two-column composition.
...
node --check composeApp\src\androidMain\assets\reader\navic-reader.js
node --check composeApp\src\androidMain\assets\reader\navic-reader-helpers.js
git diff --check -- composeApp\src\androidHostTest\kotlin\paige\navic\reader\ReaderRuntimeNavigationFlowTest.kt composeApp\src\androidMain\assets\reader\navic-reader.js composeApp\src\androidMain\assets\reader\navic-reader-helpers.js tools\reader-harness\src\run-reader-harness.mjs docs\superpowers\specs\2026-06-13-komikku-reader-port-design.md
.\gradlew.bat --no-daemon --no-build-cache --no-parallel :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeNavigationFlowTest.androidReaderUsesAdaptiveFoliatePageBoxForLargeEpubViewports"
```

Results:

- `node --check` passed for the harness and reader runtime modules.
- `git diff --check` passed for the touched files.
- Focused Android host guard passed; log: `build\codex\adaptive-page-box-focused-20260617.out.log`, exit marker: `build\codex\adaptive-page-box-focused-20260617.exit`, result `EXIT=0`.

Status:

- `Fixed locally` for avoiding Foliate's default narrow page-box cap in the reader runtime and profiler path.
- `Host/local only` for product behavior. The installed emulator APK predates this source change, so Android visual validation is still pending.
- Next emulator gate after build/install must inspect `renderer.dataset.navicAdaptivePageBox`, screenshot geometry, and page-count cache behavior on the large portrait page that previously showed excessive blank paper.

## 2026-06-17 Adaptive EPUB Page Composition Emulator Gate
...
- Exit `0`.
- Transcript: `build\codex\install-reader-dev-adaptive-page-box-20260617.out.log`.
- Screenshot: `captures\reader-dev\reader-dev-20260617-155636.png`.
- Script installed and opened `darkaxt.navic.readerdev`.
- Installed package evidence from the matrix: `versionName=v1.0.11-eta69`, `versionCode=402`, `lastUpdateTime=2026-06-17 15:56:31`.
- Foreground and readiness evidence: `mCurrentFocus=Window{... darkaxt.navic.readerdev/paige.navic.androidApp.MainActivity}` and `Reader bridge raw: {"type":"publicationReady"}`.

Matrix command:

```powershell
$expectedVersionName = (Select-String -Path 'androidApp\build.gradle.kts' -Pattern 'versionName = "([^"]+)"').Matches.Groups[1].Value
.\scripts\adb-reader-komikku-matrix.ps1 -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -ExpectedVersionName $expectedVersionName -ArtifactRoot 'captures\reader-komikku-matrix\adaptive-page-box-emulator-20260617-155733' -NoLaunch -IncludeCoverChecks -ContinueOnFailure
```

Matrix result:

- Exit `0`.
- Artifact root: `captures\reader-komikku-matrix\adaptive-page-box-emulator-20260617-155733`.
- Summary file: `reader-matrix-summary.csv`.
- Failure file: `reader-matrix-failures.txt`.
...
```json
{
  ...
  "rendererRect": { "width": 1232, "height": 1974, "left": 0, "top": 0 },
  "maxInlineSize": "1133px",
  "maxBlockSize": "1885px",
  "maxColumnCount": "1",
  "adaptivePageBox": "{\"maxInlineSize\":\"1133px\",\"maxBlockSize\":\"1885px\",\"maxColumnCount\":\"1\",\"viewportWidth\":1232,\"viewportHeight\":1974,\"flowMode\":\"paged\"}"
}
```

Visual check:

- `captures\reader-komikku-matrix\adaptive-page-box-emulator-20260617-155733\texture-next-walk\screen.png` shows a text-heavy EPUB page using most of the portrait paper height instead of the previous narrow, short centered body.
- `captures\reader-komikku-matrix\adaptive-page-box-emulator-20260617-155733\drag-next\screen.png` still shows valid empty paper on a title/section page, which is allowed by the semantic-boundary rule.

Status:

- `Emulator validated` for applying the adaptive Foliate page-box attributes on the rebuilt reader-dev APK and keeping the current scripted Komikku reader input/texture checks green.
- This does not complete the broader adaptive composition feature. Remaining work: same-section portrait page combining/spread logic, page-count cache comparison after adaptive reflow, and visual tuning against Komikku/AXN references.

## 2026-06-17 Chapter Opening Vertical Margin Correction

User-visible issue:

- After the adaptive page-box change, a chapter-opening page could still place the title too low. The reported live case was `Chapter 1` appearing around 20% down the page.
- This is distinct from the previous Foliate page-box cap. The page box was already large enough; the active EPUB document itself preserved a large publisher top margin on the opening heading.

Root cause evidence:

- Before this correction, the live WebView content frame for the reported pattern measured:
...
- The `H1` top position matched the EPUB publisher `margin-top`, not the Android shell, WebView, Foliate renderer, or adaptive page-box dimensions.
- Screenshot taken before the fix while investigating the same class of issue: `captures\reader-dev\current-emulator-before-vertical-centering-20260617-160957.png`.

Change:

- Added `readerNormalizeChapterOpeningMargins(doc, settings)` in `navic-reader-helpers.js`.
- The helper only applies in paged EPUB modes, only when the first visible content is an opening heading, and only reduces excessive top margin.
- The cap is page-relative: `min(max(innerHeight * 0.07, 56px), 144px)`. On the tested emulator content frame, the old `261.927px` margin is reduced to `131px`.
- `applyDocumentTheme(...)` now runs the helper for loaded documents.
- `scheduleCommittedRelocation(...)` now refreshes loaded content theming before posting the committed relocation, because Foliate can swap or reuse the active `about:srcdoc` content frame during navigation without firing the original load path again.

Red/green guards:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeSettingsBridgeTest.androidReaderCapsExcessiveChapterOpeningTopMargins"
.\gradlew.bat --no-daemon --no-build-cache --no-parallel :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeSettingsBridgeTest.androidReaderRefreshesContentThemeAfterRelocation"
```

Results:

- `androidReaderCapsExcessiveChapterOpeningTopMargins` red: failed before the helper/import/call existed. Green: passed after adding the helper and wiring it into `applyDocumentTheme`.
- Emulator then revealed a second miss after `goToProgress`: the active chapter frame still had `marginTop=261.927px`, because navigation changed the frame content after the load-time theme application.
- `androidReaderRefreshesContentThemeAfterRelocation` red: failed before the relocation refresh existed. Green: passed after refreshing loaded content theme before `postLocationChanged(...)`.
...
node --check composeApp\src\androidMain\assets\reader\navic-reader-helpers.js
node --check composeApp\src\androidMain\assets\reader\navic-reader.js
git diff --check -- composeApp\src\androidHostTest\kotlin\paige\navic\reader\ReaderRuntimeSettingsBridgeTest.kt composeApp\src\androidMain\assets\reader\navic-reader-helpers.js composeApp\src\androidMain\assets\reader\navic-reader.js docs\superpowers\specs\2026-06-13-komikku-reader-port-design.md
```

Results:

- `node --check` passed for both reader runtime modules.
- `git diff --check` passed for the touched files before this documentation append.
- Reader-dev APK installed and relaunched on `emulator-5554` with:

```powershell
.\scripts\install-reader-dev.ps1 -EnvFile 'C:\Users\darka\Documents\Projects\Android\Navic\bindery-debug.env' -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -Capture
```

- Install result: exit `0`; reader bridge reported `{"type":"publicationReady"}`.
- Screenshot from relaunch: `captures\reader-dev\reader-dev-20260617-164443.png`.
- The visible screenshot stayed on the native shell-cover layer, so visual screenshot evidence is not useful for this slice. CDP was used to measure the actual `about:srcdoc` EPUB content frame under the shell cover.

Emulator CDP result after fix:

```json
{
  "innerWidth": 3399,
  "innerHeight": 1878,
  "text": "Chapter 2 Now, you\u2019re probably wondering about the beginning of the previous chapter...",
  "headings": [
    {
      "tag": "H1",
      "text": "Chapter 2",
      ...
      "height": 34.66666793823242,
      "marginTop": "131px",
      "marginBlockStart": "131px",
      "capped": "true",
      "originalMargin": "261.927",
      "cap": "131"
    }
  ]
}
...
- `Emulator validated` for capping excessive publisher top margin on chapter-opening headings after navigation.
- This does not claim to solve native shell-cover visibility, content screenshot presentation, same-section page combining, or the remaining Komikku UI fidelity work.

## 2026-06-17 Post-Margin Emulator Komikku Matrix

Reason:

- The previous chapter-opening margin correction changed the EPUB runtime and was CDP-validated, but the hard emulator gate requires the scripted matrix before using it as Android reader evidence.
...
- `versionName=v1.0.11-eta69`.
- `versionCode=402`.
- `lastUpdateTime=2026-06-17 16:44:38`, after the chapter-margin runtime edits and reader-dev install.

Matrix command:

```powershell
$expectedVersionName = (Select-String -Path 'androidApp\build.gradle.kts' -Pattern 'versionName = "([^"]+)"').Matches.Groups[1].Value
.\scripts\adb-reader-komikku-matrix.ps1 -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -ExpectedVersionName $expectedVersionName -ArtifactRoot 'captures\reader-komikku-matrix\post-margin-emulator-matrix-20260617-165601' -NoLaunch -IncludeCoverChecks -ContinueOnFailure
```

Artifact evidence:

- Matrix artifact root: `captures\reader-komikku-matrix\post-margin-emulator-matrix-20260617-165601`.
- Summary file: `reader-matrix-summary.csv`.
- Failure file: `reader-matrix-failures.txt`.
- Wrapper note: the outer background exit marker was not written, matching the known matrix PowerShell exit behavior. The summary/failure files were produced and are authoritative for this run.

Baseline:

- `baseline-current-reader/screen.png` shows the loaded native shell cover for `Alcatraz versus the Evil Librarians`, not login, library, launcher, or splash.
...
Spot-checked screenshot evidence:

- `drag-next/screen.png` shows a real readable EPUB page, not the cover, with the enlarged page box, visible paper texture, and no chrome overlay.

Status:

- `Emulator validated` for the chapter-opening margin runtime change not regressing the scripted Komikku reader input/texture matrix.
- This does not complete the broader target state. Remaining work still includes faithful progress rail visual parity, same-section portrait composition/spread logic, settings sheet parity, deterministic page model coverage on more books/devices, PDF browsing/interaction parity, and AXN/Foliate capability restoration behind controlled adapters.

## 2026-06-17 Chapter Opening Tightening And Bottom Paper Reserve

Reason:

- User feedback after the first chapter-opening cap: `Chapter 1` looked better, but still slightly low compared with surrounding pages.
- User feedback after the visible opener: the title looked nicer, but dense text still reached too close to the bottom page edge.
- Follow-up requirement: verify normal resolutions as well, so the bottom reserve does not make ordinary phone pages feel cramped.

Root cause:

- CDP inspection showed no literal blank text nodes/newlines before `Chapter 1`.
- The EPUB body starts at `top=0`; the opening offset is the publisher `h1.titulocap` top margin.
- The first cap changed `261.927px` to `131px`, which was still visually generous on a large page box.

Changes:

- Tightened `readerNormalizeChapterOpeningMargins(...)` from `min(max(innerHeight * 0.07, 56px), 144px)` to `min(max(innerHeight * 0.045, 48px), 96px)`.
- Increased the adaptive Foliate block reserve from `4.5%` to `6.5%` so dense pages keep a more natural bottom paper edge instead of running text down to the page-number area.
- Added a reader harness guard requiring large portrait pages to keep `maxBlockSize <= 1860px` while still requiring normal phone pages to keep at least `800px` block capacity.

Red/green evidence:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeSettingsBridgeTest.androidReaderCapsExcessiveChapterOpeningTopMargins"
```

- Red: failed while the helper still contained the old `0.07` cap.

```powershell
node tools\reader-harness\src\run-reader-harness.mjs --mode adaptive-page-box-logic
```

- Red: failed while the large portrait page box still produced `maxBlockSize=1885px`.
...
node --check composeApp\src\androidMain\assets\reader\navic-reader-helpers.js
node --check composeApp\src\androidMain\assets\reader\navic-reader.js
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeNavigationFlowTest.androidReaderUsesAdaptiveFoliatePageBoxForLargeEpubViewports" --tests "paige.navic.reader.ReaderRuntimeSettingsBridgeTest.androidReaderCapsExcessiveChapterOpeningTopMargins" --tests "paige.navic.reader.ReaderRuntimeSettingsBridgeTest.androidReaderRefreshesContentThemeAfterRelocation"
```

- All commands exited `0`.

Computed page-box evidence:

```text
foldPortrait {"maxInlineSize":"1133px","maxBlockSize":"1846px","maxColumnCount":"1","viewportWidth":1232,"viewportHeight":1974,"flowMode":"paged"}
normalPhone {"maxInlineSize":"361px","maxBlockSize":"816px","maxColumnCount":"1","viewportWidth":393,"viewportHeight":873,"flowMode":"paged"}
landscapeFold {"maxInlineSize":"1280px","maxBlockSize":"1152px","maxColumnCount":"2","viewportWidth":1974,"viewportHeight":1232,"flowMode":"paged"}
```
...
- `versionName=v1.0.11-eta69`.
- `versionCode=402`.
- `lastUpdateTime=2026-06-17 17:55:30`.
- Reader bridge reported `{"type":"publicationReady"}` before the capture.

Live CDP evidence after reinstall:
...
```json
{
  "content": {
    "innerWidth": 3399,
    "innerHeight": 1846,
    "body": { "top": 0, "height": 1846 },
    "heading": {
      "text": "Chapter 1",
      "rect": { "top": 83, "bottom": 117.66666793823242 },
      "style": { "marginTop": "83px", "marginBlockStart": "83px" },
      "dataset": {
        "navicChapterOpeningMarginCapped": "true",
        "navicChapterOpeningOriginalMargin": "261.927",
        "navicChapterOpeningMarginCap": "83"
      }
    },
    "firstParagraph": {
      "rect": { "top": 144.33334350585938, "bottom": 165.1250114440918 }
    }
  }
}
```
...
- `captures\reader-dev\chapter1-opener-margin-tightened-visible-20260617.png` shows the tightened `Chapter 1` opener before the bottom reserve adjustment.
- `captures\reader-dev\bottom-reserve-visible-20260617.png` is not page-layout evidence; it shows the shell cover with chrome because the capture tap surfaced the cover menu instead of exposing readable content.

Matrix status:

- First matrix artifact root: `captures\reader-komikku-matrix\bottom-reserve-emulator-20260617-175900`.
- Result: invalid/blocked matrix tail. All steps through `drag-previous` passed. `texture-next-walk` failed while pulling `/sdcard/navic-reader-smoke.png`; `texture-previous-walk` then reported no ADB devices connected.
- Second matrix artifact root: `captures\reader-komikku-matrix\bottom-reserve-emulator-rerun-20260617-180120`.
- Result: invalid/blocked rerun. It started from a changed cover/chrome state, so `cover-drag-next` failed due no shell-cover swipe. The texture steps hit the same screenshot-pull/device blip.
- Current ADB state after the failed tails: `emulator-5554 device`; `sys.boot_completed=1`; installed package evidence remained `versionName=v1.0.11-eta69`, `versionCode=402`, `lastUpdateTime=2026-06-17 17:55:30`.

Status:

- `Emulator CDP validated` for the live EPUB page-box height and tighter chapter-opening margin on the rebuilt reader-dev APK.
- `Host validated` for large fold/tablet, landscape fold, and normal phone page-box math.
- `Matrix tail blocked` for texture walk screenshots due ADB capture/pull instability. Do not treat the blocked texture tail as a reader product regression without a clean rerun or script hardening.

## 2026-06-17 Serial-Aware Smoke Script Validation Fix

Reason:

- The bottom-reserve matrix tail was blocked twice by validation infrastructure, not by a reader assertion:
  - `texture-next-walk` failed while pulling `/sdcard/navic-reader-smoke.png` and left a zero-byte local `screen.png`.
  - `texture-previous-walk` then reported no ADB devices connected even though `adb devices` recovered immediately afterward.
- The smoke script still mixed `Invoke-Adb` with raw `adb` calls after `DeviceSerial` was selected, including `devices`, `wm size`, launch, `pidof`, package dump, devtools socket dump, `uiautomator`, and `logcat`.

Change:

- `adb-reader-smoke.ps1` now routes capture and diagnostic commands through `Invoke-Adb`.
- `Invoke-Adb` gained an explicit `-PassThru` switch so command output is captured only where the script needs it.
- A host guard rejects raw `adb` command execution in the smoke-script body after the helper section, while still allowing the helper itself to execute `adb`.

Red/green guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeAssetsTest.adbReaderSmokeUsesSerialAwareAdbHelperForCaptureAndDiagnostics"
```

- Red: failed while raw `adb` calls remained in the smoke-script capture/diagnostic path.
- Green: passed after converting those commands to `Invoke-Adb`.

Additional host verification:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeAssetsTest.adbReaderSmokeCapturesFocusedReaderDiagnostics" --tests "paige.navic.reader.ReaderRuntimeAssetsTest.adbReaderSmokeUsesSerialAwareAdbHelperForCaptureAndDiagnostics" --tests "paige.navic.reader.ReaderRuntimeAssetsTest.adbReaderSmokeUsesEffectiveOverrideDisplaySizeForFractionGestures" --tests "paige.navic.reader.ReaderRuntimeAssetsTest.adbReaderSmokeCanDriveEta50SwipeAndContentDiagnostics"
```
...
- Exit `0`.
- Reader bridge reported `{"type":"publicationReady"}` before the capture.
- Screenshot: `captures\reader-dev\reader-dev-20260617-182336.png`.

Clean matrix rerun:

```powershell
$expectedVersionName = (Select-String -Path 'androidApp\build.gradle.kts' -Pattern 'versionName = "([^"]+)"').Matches.Groups[1].Value
.\scripts\adb-reader-komikku-matrix.ps1 -Package 'darkaxt.navic.readerdev' -DeviceSerial 'emulator-5554' -ExpectedVersionName $expectedVersionName -ArtifactRoot 'captures\reader-komikku-matrix\serial-aware-smoke-rerun-20260617-182420' -NoLaunch -IncludeCoverChecks -ContinueOnFailure
```
...
- Exit `0`.
- Artifact root: `captures\reader-komikku-matrix\serial-aware-smoke-rerun-20260617-182420`.
- `reader-matrix-failures.txt`: `No matrix failures.`
- Passed steps:
  - `baseline-current-reader`
  - `cover-center-tap-toggle`
...
Texture walk evidence:

- `texture-next-walk/screen.png`: `5,086,187` bytes.
- `texture-previous-walk/screen.png`: `4,997,256` bytes.
- `texture-next-walk/reader-diagnostics-summary.txt`: `textureDirectionSamples=12`, `wrongTextureDirection=False`.
- `texture-previous-walk/reader-diagnostics-summary.txt`: `textureDirectionSamples=8`, `wrongTextureDirection=False`.

Status:

- `Validation path fixed and emulator validated` for the serial-aware smoke-script capture/diagnostics path.
- The previous bottom-reserve matrix blocker is closed by the clean rerun. Continue reader work from the broader Komikku/Anx parity backlog rather than re-debugging the blocked matrix tail.

## 2026-06-17 Compact Settings Tabs, Bounded Dev Capture, And Normal-Resolution Page Check

Reason:

- The reader settings sheet was drifting toward a cramped settings page on small surfaces: tab labels could wrap/truncate badly, especially around `Reading mode`, `PDF/Image`, and `Custom filter`.
- The dirty reader-dev launch script could still fail validation on noisy emulators because `Wait-ReaderDevPublicationReady` read the entire logcat buffer each loop.
- After tightening chapter-opening top margins and increasing bottom paper reserve, normal phone resolutions had to be checked so the large/fold viewport fix did not regress ordinary pages.

Change:

- `ReaderSettingsDialog.kt` now gives every settings tab a full label and optional compact label.
- The settings sheet uses compact tab labels only when the dialog width is below `520.dp`, and tab text is one-line ellipsized instead of wrapping.
- `install-reader-dev.ps1` now waits for `publicationReady` using a bounded recent logcat window: `logcat -d -v brief -t 1000`.

Red/green guards:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" --rerun-tasks :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderSettingsDialogLivesInDedicatedKomikkuComponentFile"
```

- Red: failed while the production settings sheet had no `compactTabLabels` / `useCompactLabels` path.
- Green: exit `0` after compact/full tab-label support was added.

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderProgressAndSettingsUseSharedKomikkuIntegerSliderPrimitive" --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderShellUsesKomikkuEquivalentOverlayStack" --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderChromeUsesKomikkuEquivalentSideProgressRail"
```

- Exit `0`.

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" --rerun-tasks :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderDevEnvironmentContractTest.readerDevScriptsAndSecretFilesAreDocumentedAndIgnored"
```

- Red: failed while `install-reader-dev.ps1` still used an unbounded full `logcat -d -v brief` dump.
- Green: exit `0` after adding the bounded `-t 1000` logcat window.

Reader-dev launch validation:
...
- Exit `0`.
- Reader bridge reported `{"type":"publicationReady"}` before capture.
- Screenshot: `captures\reader-dev\reader-dev-20260617-190642.png`.
- Installed dirty reader-dev build remained `versionName=v1.0.11-eta69`, `versionCode=402`; this was not a GitHub release candidate.

Fold/tablet viewport visual check:

- Emulator profile: `1848x2960 @ 240`.
- `captures\reader-progress-rail\fold-text-check-20260617.png`: landed on the EPUB title page; not used as prose layout evidence because the page is intentionally sparse.
- `captures\reader-progress-rail\fold-prose-check-20260617.png`: valid prose page, organic number `12 / 147`.
- Result: pass for the current bottom-reserve goal. The prose text keeps a readable bottom reserve and does not collide with the organic page number.

Normal phone viewport visual check:

- Emulator profile: `1080x2400 @ 420`.
- `captures\reader-progress-rail\phone-1080x2400-controlled-prose-20260617.png`: valid prose page, organic number `12 / 308`.
- Result: pass for the current bottom-reserve goal. The phone page uses the available height, keeps the page number clear, and does not show the previous large-viewport bottom crowding regression.
- `captures\reader-progress-rail\phone-1080x2400-controlled-menu-20260617.png`: valid menu-overlay capture on a prose page.
- Result: functional pass for overlay survival at normal phone size, but visual parity remains incomplete: the right progress rail is still heavier/less Komikku-faithful than the reference and remains in the Komikku UI backlog.

Invalid conventional-resolution attempt:

- Emulator profile attempted: `1080x1920 @ 420`.
- ADB dropped offline during screenshot pull/restore.
- `captures\reader-progress-rail\phone-1080x1920-prose-check-20260617.png` is zero bytes and must not be used as reader evidence.
- Emulator restored afterward to `1848x2960 @ 240`.

Status:

- `Host guards passed` for compact settings tabs and bounded reader-dev capture.
- `Emulator validated` for fold/tablet prose and normal tall-phone prose page layout.
- `Known remaining issue` unchanged: normal-phone progress rail/menu visuals are still not faithful enough to Komikku and should be redesigned in the Komikku UI parity backlog, not papered over as a finished feature.

Settings sheet visual check:

- `captures\reader-progress-rail\after-cover-center-tap-20260617.png`: cover center tap surfaced chrome over the native cover instead of immediately dismissing the cover.
- `captures\reader-progress-rail\settings-sheet-fold-20260617.png`: valid fold/tablet settings-sheet capture.
- Result: pass for the compact-tab implementation on the fold/tablet sheet. The tab labels are single-line and do not wrap; because the sheet is wide enough, it correctly uses the full labels (`Reading mode`, `General`, `Custom filter`).

Invalid phone settings-sheet attempt:

- `captures\reader-progress-rail\settings-sheet-phone-1080x2400-20260617.png`: invalid; density change recreated the reader and captured the `Preparing reader` screen.
- `captures\reader-progress-rail\phone-cover-chrome-for-settings-20260617.png`: invalid for reader settings; the follow-up phone-profile dev launch reported `No Bindery reader resource discovered; launching Bindery Books catalog` and captured the Navic splash/catalog path.
- Do not use either artifact as compact phone-tab evidence. A phone settings-sheet validation still needs a stable direct reader launch at `1080x2400 @ 420`.

## 2026-06-17 Required Reader Launch Guard And Normal-Resolution Recheck

Reason:

- The previous phone settings-sheet validation was invalid because the dirty dev launcher silently fell back to the Bindery Books catalog when it could not resolve a direct EPUB/PDF target.
- That fallback is useful for manual catalog testing, but it is wrong for reader validation: a splash, login, catalog, or library screenshot can never be accepted as evidence for Komikku reader layout.
- The bottom-reserve/page-layout work also needed another normal-resolution pass after the fold/tablet checks, including the previously invalid `1080x1920` profile.

Root-cause evidence:

- `C:\Users\darka\Documents\Projects\Android\Navic\bindery-debug.env` currently has Bindery OPDS credentials but no direct reader target keys (`BINDERY_TEST_RESOURCE_ID`, `NAVIC_READER_DEV_RESOURCE_HREF`, or `NAVIC_READER_DEV_PUBLICATION_URL`).
- Without an explicit required-reader gate, `install-reader-dev.ps1` could let discovery fail and still launch the catalog, producing a false validation artifact.

Change:

- `install-reader-dev.ps1` now supports `-RequireReaderLaunch`.
- When that switch is present and the script cannot resolve `NAVIC_READER_DEV_PUBLICATION_URL`, `NAVIC_READER_DEV_RESOURCE_HREF`, `BINDERY_TEST_RESOURCE_ID`, or a discovered EPUB/PDF resource, it throws `Reader launch target required...` instead of falling through to the catalog.
- The non-required fallback remains available for manual Bindery catalog testing.
...
- Red: failed at `ReaderDevEnvironmentContractTest.kt:139` while `install-reader-dev.ps1` had no `-RequireReaderLaunch` mode.
- Green: exit `0` after adding the required-reader switch and failure message.

Required-reader launch validation:
...
- Exit `0`.
- Discovery found `Alcatraz versus the Evil Librarians (epub)`.
- Foreground confirmed `darkaxt.navic.readerdev/paige.navic.androidApp.MainActivity`.
- Reader bridge reported `{"type":"publicationReady"}` before capture.
- Screenshot: `captures\reader-dev\reader-dev-20260617-200305.png`.

Normal-resolution visual checks:

- Profile: `1080x2520 @ 422` (`zfold7-cover`).
- Artifact: `captures\reader-progress-rail\zfold-cover-prose-deeper-20260617\screen.png`.
- Result: valid prose page at `16 / 294`; layout uses the height naturally, keeps the organic page number readable, and does not show bottom crowding.

- Profile: emulator default reset, `1080x2400 @ 420`.
- Artifact: `captures\reader-progress-rail\phone-default-1080x2400-prose-20260617\screen.png`.
- Result: valid prose/section page at `1 / 308`; section heading starts in a reasonable upper band, content uses the available height, and the organic page number remains clear.

- Profile: conventional shorter phone, `1080x1920 @ 420`.
- Artifact: `captures\reader-progress-rail\phone-1080x1920-prose-20260617\screen.png`.
- Result: valid prose/section page at `3 / 385`; this replaces the earlier zero-byte `1080x1920` artifact. The shorter viewport has less content per page, but the page number remains readable and the bottom reserve is acceptable.

Harness note:

- One `1080x2400` relaunch initially hit a transient `adb logcat -d -v brief -t 1000` exit `255` during the forced stop/start window.
- A direct follow-up logcat probe returned exit `0` and included `publicationReady`, and the repeated required-reader launch succeeded.
- Treat that as an emulator/logcat harness transient, not reader layout evidence.

Status:

- `Closed for validation harness`: direct reader checks can now require an actual EPUB/PDF target and fail rather than silently validating the catalog.
- `Normal-resolution layout rechecked`: `1080x2520`, `1080x2400`, and `1080x1920` prose captures are valid and visually acceptable for the current bottom-reserve/page-layout slice.
- `Known remaining issue`: this does not close Komikku UI parity gaps such as the progress rail shape/weight, settings-sheet redesign, richer texture visibility, or drag-preview/page-turn animation.

## 2026-06-17 TOC Href Navigation And Progress Rail Regression

User report:

- The right progress rail did not respond as expected: the forward control did not jump to the next chapter.
- After using the chapter browser to jump to Chapter 1, later chapter-browser selections no longer changed the visible chapter.
- The reader still showed Author's Foreword after those actions, so the progress rail continued to operate against the stale Author's Foreword state.

Root-cause evidence from installed `v1.0.11-eta69` on the emulator:

- Kotlin/controller dispatched `goToHref(OEBPS/Text/authorsforeword.xhtml)`, then later dispatched `goToHref(OEBPS/Text/capitancebolleta02.xhtml)`.
- The reader runtime continued logging content layout at the same section index and text sample after the later `goToHref`.
- The bridge logged `location-changed:duplicate-skipped go-to`, consistent with Navic committing the previous relocation detail after a raw Foliate navigation call that did not actually move to the requested section.
- Follow-up progress-rail commands were `goToChapterProgress(OEBPS/Text/authorsforeword.xhtml, ...)`, which is correct given the stale current chapter but wrong for the user's requested destination.

Fix direction:

- Treat this as a failed href-navigation handoff, not a progress-rail rendering bug.
- The native chapter forward/back callbacks already have host guards that route through TOC chapter navigation instead of page turns.
- The JS bridge now resolves explicit href/CFI navigation through Foliate's `resolveNavigation` / `book.resolveHref` and calls `renderer.goTo` with the resolved section target before committing a Navic location update.
- `view.goTo` remains the fallback only when a resolved renderer target is unavailable.

Red/green guard:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" --rerun-tasks :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeNavigationFlowTest.androidReaderResolvesTocHrefNavigationBeforeCommittingLocation"
```

- Red: failed at `ReaderRuntimeNavigationFlowTest.kt:59` while `goTo(locator)` delegated directly to `this.view.goTo(locator)` and then committed the last relocation detail.
- Green: included in the focused reader-navigation host test run below after adding `resolveReaderNavigationTarget(locator)` and the resolved `renderer.goTo` path.

Verification:

```powershell
.\gradlew.bat --no-daemon --no-build-cache --no-parallel "-Pkotlin.incremental=false" --rerun-tasks :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeNavigationFlowTest.androidReaderResolvesTocHrefNavigationBeforeCommittingLocation" --tests "paige.navic.reader.ReaderControllerTest.chapterNavigatorArrowsNavigateAdjacentTocEntriesInsteadOfTurningPages" --tests "paige.navic.reader.ReaderControllerTest.chapterNavigatorArrowsDisableAtTocBounds" --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderChapterNavigatorArrowsUseTocChapterNavigationCallbacks" --tests "paige.navic.reader.ReaderRuntimeShellProgressTest.commonReaderSurfaceExposesPageTurnControlsOutsideChapterNavigator"
```

- Exit `0`.
- Confirms the native chapter navigator arrows route to adjacent TOC entries and the runtime no longer blindly commits stale relocation state after raw href navigation.
...
- `Locally fixed, not device-validated yet`: host/static guards and JS syntax checks pass.
- `Next validation gate`: install a dirty reader-dev build or release candidate, then use the chapter browser to jump away from Author's Foreword, verify the visible section changes, and verify the progress rail's forward/back buttons use the new current chapter rather than stale Author's Foreword.

## 2026-06-17 Manual Progress Rail Follow-up

Manual result after dirty reader-dev validation:

- The previous stale-TOC symptom improved enough that chapter forward could work from Author's Foreword, but the progress rail is still not correct.
- Progress rail forward/back buttons still do not work when the reader is on page 1.
- In Author's Foreword, tapping/dragging the progress rail only reaches page `2 / 4`; it does not reach the final two chapter pages through the rail.
- In Chapter 1, the progress rail only reaches page `10 / 12`; it does not reach the final two chapter pages there either.
- Dragging backward in a chapter caused an emulator/app disruption that looked related to system gesture navigation; after restoring, the book reopened at the first page and did not recover the previous location.
- On the cover, the bottom menu appears over the cover.

Working diagnosis to investigate:

- This is now a chapter-local progress boundary bug, not only the earlier raw `goToHref` stale-section bug.
- The repeated “last two pages are unreachable” pattern points at progress fraction/page-index conversion, slider endpoint mapping, or a rail geometry clamp that never emits chapter fractions close enough to the end of a chapter.
- The page-1 button failure must be checked separately from the slider endpoint bug: chapter arrows may be disabled from stale TOC/current-position state, or they may be hit-tested under the cover/menu layers.

Next required gate:

- Inspect live ADB logs around progress rail clicks and chapter arrows.
- Add host guards for chapter progress endpoint mapping and page-1 chapter-arrow enablement before changing production code.
- Do not publish a release for this slice until the rail can be validated past the final-page boundary in at least Author's Foreword and Chapter 1.

## 2026-06-17 Controlled Relocation Commit Fix

ADB evidence from the dirty reader-dev build:

- Native dispatched repeated `goToChapterProgress(OEBPS/Text/capitancebolleta01.xhtml, 0.9090909090909091)` commands while testing the Chapter 1 progress rail.
- The JS bridge logged `chapter-progress-seek:start`, but the subsequent committed location was posted as generic `reason=relocate-committed`.
- Because the reason was generic, `page-number:passive-relocate-clamped` activated and rewrote a candidate jump such as `candidate=24` down to `to=23`.
- That means the progress rail endpoint seek was being treated like passive page-turn fallout, which explains why rail clicks/drags could stall before the final pages.

Root cause:

- Foliate emits `relocate` during `renderer.goTo(...)`.
- Navic's `onRelocate(detail)` scheduled the commit immediately with the default `relocate-committed` reason.
- The explicit `goToChapterProgress(...)` call scheduled `chapter-progress-seek` only after the Foliate call returned, too late to win if a relocate was already queued.

Fix:

- Add `beginControlledRelocation(reason)` / `consumeControlledRelocationReason(fallback)` to the JS runtime.
- Arm `go-to`, `progress-seek`, and `chapter-progress-seek` before calling Foliate navigation methods.
- Make `onRelocate(detail)` schedule with the consumed controlled reason instead of always scheduling a generic `relocate-committed`.
- Keep passive one-page clamping available for actual unowned relocation events, but prevent it from rewriting explicit chapter rail seeks.

Red/green guard:

```powershell
.\gradlew.bat --no-daemon --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeShellProgressTest.androidReaderChapterRailSeekCommitsWithControlledReasonInsteadOfPassiveClamp"
```

- Red: failed at `ReaderRuntimeShellProgressTest.kt:117` before production changes because the runtime had no controlled relocation reason path.
- Green: exit `0` after adding controlled relocation ownership around Foliate navigation.

Cover chrome follow-up:

- User reported that the bottom menu appears over the cover.
- Code inspection confirmed the chapter navigator was hidden by `showChapterNavigator = visible && !controllerState.shellCoverVisible`, but the bottom bar itself still used `visible = visible`.
- Added `showBottomBar = visible && !controllerState.shellCoverVisible` so the whole bottom menu is suppressed while the native shell cover is visible.

Red/green guard:

```powershell
.\gradlew.bat --no-daemon --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeCommonChromeTest.commonReaderBottomMenuDoesNotRenderOverShellCover"
```

- Red: failed at `ReaderRuntimeCommonChromeTest.kt:536` before production changes.
- Green: exit `0` after the bottom bar used `showBottomBar`.
...
- `Locally fixed, not release-published`: the rail seek should now commit with `chapter-progress-seek` and avoid passive clamping.
- `Needs dirty APK validation`: verify Author's Foreword reaches its final page through the rail, Chapter 1 reaches its final page through the rail, page-1 chapter arrows respond, and cover mode no longer shows the bottom menu.

## 2026-06-17 Manual Progress Rail And State Recovery Follow-up

Manual result after the latest reader-dev/APK check:

- The progress rail previous/next buttons still do not work when positioned on page `1`.
- In Author's Foreword, progress rail clicking only reaches `2 / 4`; the last two chapter-local pages are still unreachable through the rail.
- In Chapter 1, progress rail clicking only reaches `10 / 12`; the last two chapter-local pages are still unreachable through the rail.
- Dragging backward in a chapter triggered an emulator/app disruption that appeared related to system gesture navigation; after restoring the window, the book reopened from the first page and did not recover the previous location.
- On the cover, the bottom menu still appears on top.

Required interpretation:

- The controlled-relocation/passive-clamp patch is not enough by itself. The remaining rail defect must be treated as an endpoint/page-count/navigation contract bug until emulator/DevTools evidence proves otherwise.
- State recovery after a disrupted drag is a separate persistence/resume bug and must be tested with a crash/disruption path, not inferred from normal page-turn behavior.
- Cover chrome layering remains an Android UI validation failure until the installed APK that contains `showBottomBar = visible && !controllerState.shellCoverVisible` is proven on the emulator/device.

Next required gate:

- Validate the current dirty worktree/APK with DevTools and ADB before adding another workaround.
- Add a failing host guard for the unreachable final chapter pages using the actual Foliate paginated section math before production changes.
- After any production fix, rerun the focused host guards and the emulator reader-dev path, then update this section with the exact artifact root and result.
...
- Red: failed at `ReaderRuntimeShellProgressTest.kt:122` while the runtime could still let generic relocate events overwrite an already-pending explicit chapter rail reason.
- Green: exit `0` after `scheduleCommittedRelocation(...)` preserved an existing explicit pending reason when later generic `relocate-committed` events arrived before the scheduled commit fired.
- Remaining validation: install the dirty reader-dev APK and repeat the DevTools `goToChapterProgress(..., progress: 1)` check. The expected log must show the final `location-page-model reason=chapter-progress-seek`; if it still lands at `2 / 4` or `10 / 12`, the next root cause is the Foliate section endpoint/page-count mapping, not the pending reason downgrade.

Dirty APK DevTools result after the pending-reason fix:

- Installed `darkaxt.navic.readerdev` remained `versionName=v1.0.11-eta69`, `versionCode=402`, but `lastUpdateTime=2026-06-17 22:24:17`.
- Launch script reached `publicationReady`; screenshot: `captures\reader-dev\reader-dev-20260617-222423.png`.
- DevTools source check confirmed `preserveExplicitReason`, `readerRelocationReasonIsExplicit`, and `scheduleControlledRelocationFallback('chapter-progress-seek')` were present in the live WebView.
- Result: first committed locations now use `reason=chapter-progress-seek`, but endpoint seeks still land at Author's Foreword `2 / 4` and Chapter 1 `10 / 12`.
- Evidence: Foliate `paginator.js` computes paginated text page progress with `pages - 2`, while Navic was counting `Math.round(pages) - 1`; the missing final pages were sentinel columns being counted as reachable chapter pages.

Sentinel-count guard:

```powershell
.\gradlew.bat --no-daemon --no-parallel "-Pkotlin.incremental=false" :composeApp:testAndroidHost --tests "paige.navic.reader.ReaderRuntimeShellProgressTest.androidReaderReportsDynamicReflowablePagePositionToChrome"
```

- Red: failed at `ReaderRuntimeShellProgressTest.kt:694` while production still expected `Math.round(pages) - 1`.
- Green: exit `0` after introducing `reflowablePaginatedTextPageCount(pages)` and using Foliate's `Math.round(pages) - 2` contract in both live section page reporting and pagination-profile sampling.
- Remaining validation: rebuild/install reader-dev again and confirm endpoint seeks now report terminal chapter pages rather than `2 / 4` and `10 / 12`.

Focused host verification after the sentinel-count fix:
...
- First run failed at `ReaderRuntimeNavigationFlowTest.kt:66` because the stale guard still expected direct `scheduleCommittedRelocation`; the runtime now correctly uses `scheduleControlledRelocationFallback('go-to')`.
- Green: exit `0` after updating that guard to require resolved `renderer.goTo(...)` before the controlled fallback.

## 2026-06-17 Late Progress Rail Regression Notes

Manual result from the latest emulator check:

- Progress rail previous/next buttons still do not work when the reader is on page `1`.
- Progress rail clicking reached only `2 / 4` in Author's Foreword and `10 / 12` in Chapter 1 in that installed build.
- Dragging backward in a chapter disrupted the emulator/app window, likely through system gesture navigation; after restoring, Navic reopened the book from the first page and did not recover the prior reading location.
- On the cover, the bottom menu appears over the cover.

Interpretation for the next pass:

- Treat page-1 rail buttons as a separate native-controller/chrome bug from the already investigated Foliate sentinel-column endpoint bug.
- Treat the resume failure after a disrupted drag as a persistence bug: progress must be persisted on committed reading-location changes, not only on clean exit.
- Treat cover bottom-menu visibility as an installed-build validation failure until ADB screenshots from the current candidate prove the fixed native shell state.
- Do not ship another release candidate until the focused emulator checks have verified the page-1 rail buttons and cover chrome behavior on the same build.

Dirty emulator validation after the sentinel-count fix:

- Device: `emulator-5554`; package `darkaxt.navic.readerdev`; installed build reported `versionName=v1.0.11-eta69`, `versionCode=402`, `lastUpdateTime=2026-06-17 22:37:29`.
- Start state screenshot: `captures\reader-dev\reader-dev-20260617-current-before-rail.png` showed Chapter 1 at the corrected endpoint `11 / 11`.
- DevTools command `goToChapterProgress(OEBPS/Text/capitancebolleta01.xhtml, 0)` moved Chapter 1 to page `1 / 11`; screenshot: `captures\reader-dev\reader-dev-20260617-ch1-page1-before-buttons.png`.
- Tapping the lower rail chapter button from Chapter 1 page `1 / 11` dispatched `goToHref(OEBPS/Text/capitancebolleta02.xhtml)` and moved to Chapter 2 page `1 / 11`; screenshot: `captures\reader-dev\reader-dev-20260617-ch1-page1-after-bottom-rail-button.png`.
- Tapping the upper rail chapter button from Chapter 2 page `1 / 11` dispatched `goToHref(OEBPS/Text/capitancebolleta01.xhtml)` and moved back to Chapter 1 page `1 / 11`; screenshot: `captures\reader-dev\reader-dev-20260617-ch2-page1-after-top-rail-button.png`.
- Tapping the Chapter 1 rail track near the bottom emitted `goToChapterProgress(OEBPS/Text/capitancebolleta01.xhtml, 1.0)` and reported `chapterPageIndex=10`, `chapterPageCount=11`; screenshot: `captures\reader-dev\reader-dev-20260617-ch1-page1-after-rail-endpoint-tap.png` displayed `11 / 11`.

Current interpretation:

- On the dirty emulator build, page-1 chapter buttons and Chapter 1 rail endpoint are behaving correctly after the sentinel-count fix.
- The user-visible `10 / 12` / button-stuck report still needs validation on the next release candidate or the exact device/package where it was observed.
- The remaining confirmed risks are cover chrome layering on the installed build, resume persistence after disruption, and whether the same rail behavior survives a clean APK install/relaunch path.
