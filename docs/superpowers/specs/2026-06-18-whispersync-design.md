...
Focused file-owned implementation slices are tracked in `docs/superpowers/plans/2026-07-02-reader-whispersync-focused-file-plans.md`.
...
- Audio-position-driven overlay commands now suppress redundant WebView navigation when the target sidecar text range is already visible in the current rendered document. In that case the runtime logs `media-overlay-follow:already-visible`, posts `overlayFragmentActive`, and highlights in place instead of issuing another `goTo(..., 'media-overlay-follow')` after a native page turn.
...
- 2026-07-03 focused implementation tightened the interaction contract: short taps remain Komikku/native UI only, EPUB text-offset sync is routed through explicit long press (`whispersyncTextLongPress`) only, Play now seeks from the current visible EPUB text range before starting audio, reader-exit pause wiring has a source guard, and the character-offset WebView probe now asserts real DOM media-overlay marker creation/cleanup. Execution details are recorded in `docs/superpowers/plans/2026-07-02-reader-whispersync-focused-file-plans.md` under Focused Plan S.
- 2026-07-03 guardrail update: the bridge decoder now rejects legacy short-touch content claim sources (`link-touch`, `media-touch`) instead of accepting them as generic ebook interaction metadata. Short tap is native UI/navigation only; long press is the only gesture allowed to request ebook text offsets, link activation, image state changes, or sentence-to-audio seek.
- 2026-07-03 guardrail update: the bridge decoder also rejects malformed `whispersyncTextLongPress` events unless their source explicitly identifies a long-press path. Known short-tap sources (`native-short-tap`, `reader-center-tap`, `link-touch`, `media-touch`) and ambiguous/missing sources (`link`, `image`, `click`, `unknown`, or absent `source`) are discarded. This keeps sentence-to-audio seeking long-press-only even if a future runtime regression emits the wrong source.
- 2026-07-03 focused implementation now also carries an explicit `ReaderWhispersyncActiveSegmentDiagnostic` from the active ASR segment decision through `ReaderWhispersyncPlaybackPositionStep -> ReaderControllerStep -> ReaderCoordinatorStep -> ReaderScreen`. `ReaderScreen` logs `Whispersync activeSegment ... applyMediaOverlay=...`, giving ADB validation a concrete cue-selection marker to pair with `ApplyMediaOverlay` and `overlayFragmentActive`.
...
- The Audiobooks hub now surfaces a dedicated "Whispersync ready" carousel sourced from the already-loaded Audiobooks OPDS row. It reuses the exact ready-pair predicate from cover badges and does not include summary-only `whispersyncStatus`, pending pairs, or ready pairs without `artifactHref`.
...
- Current-source debug validation was rerun again after merging the current `fork/master` artwork pipeline commits into the reader branch: `captures\reader-whispersync-enjoyment\stage5c3-whispersync-enjoyment-20260630-161200` passed page-scoped control, audio-follow suppression, character-offset overlay, and exact companion-progress probes through installed `darkaxt.navic.readerdev` `v1.0.11-theta25` using `bindery-debug.env`.
- Current-source debug validation was rerun again after the theta29 reader texture/layout release: readerdev rebuilt/installed, `captures\reader-dev\reader-dev-20260701-073402.png` reached `publicationReady` for production book `3809`, and `captures\reader-whispersync-enjoyment\stage5c3-whispersync-enjoyment-20260701-073309` passed page-scoped control, audio-follow suppression, character-offset overlay, and exact companion-progress probes through `darkaxt.navic.readerdev` using `bindery-debug.env`.
- Source-aware audio-follow visible range suppression is host-tested and readerdev-emulator-proven with a non-duplicate `visibleTextRange(source=media-overlay-follow)` bridge event. The 2026-07-02 readerdev pass also proves an already-visible cue range posts `overlayFragmentActive` and logs `media-overlay-follow:already-visible` without requiring a second runtime `goTo`; next/previous texture-direction probes after that path reported `wrongTextureDirection=False`.
- Character-offset ASR overlay highlighting is readerdev-emulator-proven with a direct WebView probe.
- Current-source debug validation was rerun after the Plan M rail-only Whispersync isolation change: `captures\reader-whispersync-enjoyment\plan-n-post-rail-isolation\stage5c3-whispersync-enjoyment-20260702-092700` rebuilt/installed `darkaxt.navic.readerdev`, reached `publicationReady`, and passed page-scoped control, audio-follow suppression, character-offset overlay, and exact companion-progress probes through `bindery-debug.env`.
- Current-source debug validation was rerun after the live Audiobooks hub route diagnosis: `captures\reader-whispersync-enjoyment\stage5c3-whispersync-enjoyment-20260702-215952` passed page-scoped control, audio-follow suppression, character-offset overlay, and exact companion-progress probes for production book `3809`; the release-facing fix is to prefer `/opds/formats/audiobook` over `/opds/audiobooks` for the Audiobooks hub row when both root links exist, because only the book catalog route carries ready `syncPairs`.
...
- Current-source active-segment diagnostics are now readerdev/emulator-captured in the strengthened Stage 5C.3 pass: `captures\reader-whispersync-enjoyment\plan-s-native-playback-feedback\stage5c3-whispersync-enjoyment-20260703-034037` rebuilt/installed `darkaxt.navic.readerdev`, tapped the native `Play Whispersync audiobook` headset control after a cue-covered page probe, waited until `Pause Whispersync audiobook` was visible, and captured `Whispersync activeSegment`, `ApplyMediaOverlay=true`, and repeated `overlayFragmentActive` bridge events in the same paired playback flow. Signed-release validation still needs to prove this on the public package before a release-level usability claim.
- Current-source Whispersync implementation was re-audited after the short-tap bridge guard: targeted host tests for parser/model, schema contract, visible-range seek, playback preseek, lifecycle pause, and short-tap ownership passed, and `captures\reader-whispersync-enjoyment\plan-s-post-bridge-guard\stage5c3-whispersync-enjoyment-20260703-075029` rebuilt/installed `darkaxt.navic.readerdev` and passed page-scoped control, audio-follow, character-offset DOM overlay, and companion-progress probes for production book `3809`. The run includes `Whispersync play preseek`, repeated `Whispersync activeSegment ... ApplyMediaOverlay=true`, progressing `overlayFragmentActive` events, and `Pausing Whispersync audiobook on reader exit`.
- Current-source Whispersync implementation was re-audited again after the stricter explicit-long-press decoder guard: targeted host tests for parser/model, schema contract, visible-range seek, playback preseek, lifecycle pause, and short-tap ownership passed, and `captures\reader-whispersync-enjoyment\plan-s-post-explicit-longpress-guard\stage5c3-whispersync-enjoyment-20260703-083117` rebuilt/installed `darkaxt.navic.readerdev` and passed page-scoped control, audio-follow, character-offset DOM overlay, and companion-progress probes for production book `3809`. The run includes `Whispersync play preseek`, repeated `Whispersync activeSegment ... ApplyMediaOverlay=true`, progressing `overlayFragmentActive` events, `Pausing Whispersync audiobook on reader exit`, and a live DOM marker with class `navic-active-overlay-fragment navic-media-overlay-range` for `OEBPS/xhtml/Authorforeword.xhtml:27-75`.
- Standalone live WebView DOM proof was refreshed in `captures\reader-dev\whispersync-char-offset-overlay-live-20260703-baseline`: the DevTools probe created a `navic-active-overlay-fragment navic-media-overlay-range` marker for `OEBPS/xhtml/Authorforeword.xhtml:27-75` and `clearOverlay` left no probe-created marker behind.
