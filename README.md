...
6. Optionally enable `Picture-in-Picture` or `Landscape video mode` on Android. `Pause music while clips play` and `Remember clip position` are on by default and can be turned off.
