...
Open Settings -> Integrations -> `Aurral`, enable the integration, enter your Aurral URL, and optionally enter the Aurral username/password if your backend requires auth. Use `Test connection` to verify the endpoint, then check `Service status` and `Acquisition queue` for health, Flow, discovery, Lidarr, and request diagnostics. Artist pages with MusicBrainz artist ids can show Aurral previews, missing albums, request buttons, acquisition progress strips, request status, and similar artists from this configured connection. The next Aurral slices are a native hub, Flow/station creation, and Flow playback.
