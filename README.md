...
* Collection shuffle now keeps the full shuffled song set instead of applying a queue cap, while queue rows use stable occurrence keys so duplicate songs can remain visible and reorderable in the lazy queue UI. Collection-level shuffle also keeps the canonical queue order and lets the media player own shuffle state, so playing a shuffled artist or album does not double-shuffle the queue.
...
* LidaClips matches are cached locally before Now Playing video features activate. Once cached, clips can play as a muted cropped Now Playing video background by default; the movie button beside Lyrics only appears after a verified clip resolves, promotes the clip into the artwork area with a short artwork-to-video crossfade, starts it at the equivalent song-progress percentage, mutes the underlying song, and lets the video provide audio while Navic's normal controls remain visible. The Lyrics action follows the same verified-only rule and stays hidden until lyrics resolve for the current song.
...
* Kreate-inspired Now Playing toolbar position can be Top, Bottom, or Hidden for a cleaner fullscreen player, with the collapse control kept as a separate top-right affordance instead of sharing the bottom action strip.
...
* Kreate-inspired Now Playing technical info style shows the compact format/sample-rate/bitrate badge as a centered artwork-area overlay, with optional source stats such as bit depth, channel count, file size, and ReplayGain.
...
* On scrollable library, detail, search, activity, Aurral, and Bindery screens, the first Back action scrolls back to the top and restores the bottom/navigation chrome before a later Back exits the view.
...
* The Library screen can surface a local `Most played` shortcuts row for artists, genres, albums, playlists, and `[A]` stations that were explicitly played from Navic. Shortcuts are ranked by effective playback duration instead of raw taps, open the native destination for that entity, and resolve artist shortcut photos through the configured artwork-source priority. Aurral-first is the default, but native-first and native-only modes are available under Aurral settings. Resolved artist-photo URLs are persisted in the local cache database, reused by Most played and artist grid cards after app restarts, and ignored without deletion when Aurral is disabled.
* Configured Bindery OPDS 2 audiobooks use a dedicated Audiobooks bottom-bar context instead of inserting audiobook rows into the music Library. The compact root bar is `Library / Audiobooks / Activity`, the music context expands to `Library / Albums / Playlists / Artists / Audiobooks / Activity`, and the audiobook context expands to `Library / Audiobooks / Books / Collections / Authors / Activity`. Switching between the music and audiobook contexts animates the bottom-bar tab set instead of snapping between unrelated modes, selected audiobook tabs use the same motion feedback as music tabs, and top-bar search follows the active context so Audiobooks searches Bindery books, findings, collections, and authors instead of music. The `Audiobooks` destination opens a Library-style hub with shortcut tiles and OPDS-backed rows such as Recently added, Audiobooks, Authors, Collections, and Wanted. Rows such as Last read, Most popular, and Genres are supported when the Bindery OPDS root advertises matching catalog links, but Navic does not fabricate those feeds locally and does not surface Findings as a top-level row. Books use OPDS publications, load book-like catalogs in five-item pages as the grid is reached, and render with portrait book-cover cards instead of square album art. Bindery book and collection grids use a dedicated five-column minimum density, configurable under Settings -> Integrations -> Bindery from 5 to 8 columns without changing music album grids. Collections use `/opds/collections` with portrait collection cards, local member count, year range, and collection member order when OPDS exposes it; provider/source labels such as Hardcover are kept out of normal content rows. Authors use OPDS author catalogs and their advertised artwork, and author detail pages can load the advertised `/opds/authors/{id}/collections` catalog as a native Collections row. Author publications are sorted by publication year and require usable year metadata before they appear in that visible row. Author and Collection cards open dedicated detail pages instead of plain filtered book grids; protected cover URLs are loaded with the configured Bindery API key. A global Bindery language filter applies OPDS `owned=1`, `languages`, and `coverage=any` to audiobook, book, author, collection, search, and hub feeds so top-level surfaces only show entries with at least partial ownership in the selected language, and Navic no longer upgrades plain book cards to available just because a book-level acquisition link exists. Detail pages request selected-language availability without hiding missing publications, then gray missing books and use the same green/yellow/red availability dot language as music ownership indicators. Series stays a separate future shelf instead of being used as the user-facing Collections substitute. Collection cards use OPDS collection image metadata when present and lazily fall back to the first book cover from the visible collection detail. OPDS catalog, manifest, resource, and per-book finding metadata is cached in the local database with a fresh-first/stale-fallback policy so returning to Bindery screens does not feel like reloading a web page. Disabling Bindery removes the audiobook tabs and suppresses stale audiobook rows/routes without deleting cached settings, artwork, or metadata.
* Bindery book cards, Audiobooks hub rows, search results, and author/collection publication entries open a native Book landing page. The page keeps book covers in portrait format, shows OPDS title/author/year/duration metadata, renders available descriptions and subject chips that open audiobook-scoped search, and embeds the advertised per-book Findings feed when Bindery exposes usable source candidates. Book findings are grouped into quality-sorted Audiobooks and Ebooks lists instead of inert duplicate version rows: they prefer explicit OPDS format/language/provider/publisher metadata, fall back to bracketed publisher/edition labels only when needed, show provider, publisher/edition, format, narrator, duration, and size where available, filter locally by the selected Bindery language, hide unknown/request-only finding metadata until remote-request support exists, and open the native Finding detail page from the info action. Book landing pages do not expose direct download/request-download controls; advertised OPDS download-request actions stay attached to the native Finding detail page where source evidence and mapped files are visible. Audiobook play actions now open a native standalone player that reuses Navic's cover-blended background treatment, protected Bindery cover headers, Media3-backed OPDS audio resources, chapter progress, chapter index navigation, sleep timer entry, Audible-style speed selection, and fixed -30 / -10 / play-pause / +10 / +30 transport controls. Ebook read actions remain routed to the finding detail until native ebook playback surfaces are implemented. Finding detail pages mirror Bindery's provider evidence with cover art, provider/source metadata, mapped books, file inventory, provider notes, source links, and advertised OPDS actions. Author detail sections now align their Collections and Publications headers and rows to the same content edge as the rest of the Audiobooks surfaces.
* The Library screen can surface local `Newest` and `Starred` album rows when those collections have data, giving direct carousel access to existing Navidrome album lists without adding new recommendation state.
* Artist pages include a dedicated Shuffle action beside Play. Last.fm biographies expand below the artwork header and collapse inline, so long text scrolls with the artist page instead of growing upward over the hero. The external Last.fm page stays in the secondary overflow action. If a Last.fm API key is configured, artist pages can show a local-song `Most popular tracks` row after `Frequently played`. Artist headers avoid raw protected artist-image URLs and use the configured artwork-source priority: cached verified Aurral photos can win by default, native Navidrome cover art can be preferred, or external artwork can be suppressed entirely when Aurral is disabled or native-only is selected. Song-level artist links now reconcile stale Navidrome artist ids against the current artist table, fall back to Aurral name lookup when a contributor is not local, and route compound album artist credits to a prefilled music search so each individual artist can be opened from the filtered result set.
...
* Kreate-inspired search history controls: recent searches persist across app restarts, can be cleared from Search, and `Pause search history` under Data & Storage hides history while stopping newly submitted queries from being recorded. The main Search screen queries Navidrome plus configured Aurral artist and album search, with local results kept separate from Aurral discovery/catalog results.
...
* All song downloads route through the same download queue, including individual songs, albums, playlists, stations, artists, full-library downloads, retries, and startup recovery. If the app process restarts while songs are queued, Navic resumes queued songs that still exist in the synced library and clears stale queued entries. The queue dialog can inspect queued, downloading, and failed songs, cancel pending work, clear the whole active/failed download queue without deleting completed downloads, and retry failed downloads that still exist locally. The Activity tab also exposes failed-download retry/discard and full queue-clear actions directly from the Navic download queue card. LidaClips video cache/acquisition work has its own local queue in Activity with the same retry, discard, cancel, and clear controls instead of relying on backend dashboard rows. `Parallel downloads` defaults to 3 songs at a time and can be adjusted under Data & Storage. The scheduler keeps a worker pool behind that cap so queued songs backfill as soon as any active download finishes, and cancelled or cleared queued rows stop waiting for a slot.
* Data & Storage shows persistent LidaClips offline video clips separately from the temporary video cache. Clearing all downloads also clears those persistent clips because they are paired with offline music. Settings -> Data & Storage -> Danger Zone -> `Clear LidaClips video cache` clears the temporary verified-video cache without deleting saved offline clips.
...
* The Music Video action stays hidden until the current song resolves to an actual LidaClips clip, so lookup misses or pending lookups no longer lead to an empty clip player.
...
* The bottom `Activity` tab centralizes live integration work across Navic song downloads, Aurral album acquisitions and Flow track states, and LidaClips clip downloads. It can retry, discard, or clear Navidrome download queue rows, Aurral acquisition rows expose cancel/retry actions when the backend provides enough album or artist identifiers, and the LidaClips card now shows Navic's local clip cache/download queue rather than the backend dashboard queue. Integration health checks and backend diagnostics stay in Settings -> Integrations, so Activity remains focused on actionable client queue work.
* Native Aurral discovery loads Aurral's Discover/library sources as separate first-class collections instead of one flattened row: `Recently discovered`, `Recent releases`, `Recommended for you`, `Based on your library`, `Global picks`, dynamic `Because you like ...` genre rows, and `Explore by tag`. Library renders those artist, album, genre, and tag rows below local Navidrome rows, keeps a native Discover placeholder visible while the first discovery request is loading, filters image-less discovery cards out of Library rows, and opens each artist collection's `See all` action into that specific native grid instead of one generic merged list. `Explore by tag` uses a compact wrapping tag wall in Library and the native hub, renders every available top tag directly instead of using the normal preview cap, and no longer shows a redundant `See all` link for the tag wall. Backend-provided fallback genre sections are parsed from Aurral's current `name` field, and generated top-genre sections are no longer arbitrarily capped, so every Aurral genre collection with matching artists can surface in Library and the native hub. The native hub shows the same buckets with row-specific expansion for monitoring/opening artists, opening recent-release albums, and drilling into tag-filtered artist grids. Artist rows still jump to the native local Artist detail when a matching Navidrome artist exists; otherwise they open the native Aurral artist/catalog page and can be sent to Aurral monitoring when the configured Aurral user has artist-add permission. Missing Discover artist artwork is hydrated from exact Aurral artist search matches without replacing the recommendation identity. Library discovery rows, the native Aurral hub, Aurral search, expanded Discover grids, and similar-artist candidates reuse the persistent artist-photo cache so Aurral-priority artist images stay consistent across `Based on your library`, local artist pages, and external Aurral artist pages. The Artists page has a `+` action for native discovery artist search backed by `/api/search/artists`, and the search rows use resolved artwork plus recommendation/catalog context. Album search backed by `/api/search?scope=album` is included in the main Search screen and remains available in the native hub; owned results with a local album id open the native album detail page, while missing results open the same album-like missing-album detail page used by artist catalogs. Album search results and recent-release rows are deduped and sorted by release year from newest to oldest, with undated results last. Stations has a `+` action for creating new Aurral Flows, while the hub remains available internally for detailed Flow operations: enable or disable existing Flows, trigger a Flow run when the backend exposes the required permissions, play the matching synced Navidrome `[A]` Station directly when it has songs, play ready Flow jobs directly from Aurral while the Station has not synced yet, and jump to that Station detail page when one already exists locally.
...
* Library Aurral rows and the native Aurral Discover grid refresh through a fast discovery-only path, so recommendation collections are not hidden behind slower service-status diagnostics or missing-artwork hydration. Aurral discovery, library-artist, artist/album search, artist-enrichment, album-track, and release-group-cover metadata is persisted locally with a fresh-first/stale-fallback policy, and successful acquisition or monitor mutations clear the affected Aurral metadata cache so returning to music discovery screens reuses local data without hiding backend updates.
* Artist pages enrich local Navidrome artists with Aurral data by trying the local MusicBrainz artist id before weaker Aurral Discover name matches, but they fall through to verified Discover identities when the first candidate cannot confirm monitoring state. Artist headers prefer verified Aurral imagery before stale fallback-style local covers, then use the synced artist image URL. Native Aurral hub and artist/catalog pages use the same detached mini-player bottom overlay as the main Library and Artist pages so the bottom transition remains a fade instead of a hard cut. The artist biography stays below the artist name in collapsed and expanded states; expanded text uses a bounded internal scroll instead of jumping layout or overflowing upward, with top/bottom fade gradients that appear only when there is hidden text in that scroll direction. Local albums and Aurral-only missing albums are merged into one newest-first Albums row, local albums win duplicate matches, missing albums use grayscale resolved covers, existing request status is shown, small acquisition progress strips appear over requested and locally matched album covers, and album/preview cards show compact status dots: green for owned, orange for partial/requested, and red for missing/unrequested. Library album and song cards use the same local status dots when discovery integration is configured, while Library and discovery artist cards use the verified monitor state from the artist page instead of guessing. Aurral library-artist monitor state is cached per connection for 10 minutes to avoid repeated Lidarr checks on rows that are already in the library. Monitor/unmonitor actions now enter an in-memory confirmation queue: the first request is sent immediately, the Aurral logo switches to a queued spinner badge, Navic polls Aurral's library artist endpoint for confirmation, silently re-emits the request every few minutes while it remains pending, updates the cached row state only after verification, and shows a snackbar if the backend request fails. The monitor-state icon set uses the Aurral logo directly: plain for monitored, crossed for verified not monitored, question-badged for unknown/unverified, and spinner-badged for queued confirmation. One unified similar-artists row merges external image/match data with local Navidrome similar artists. Similar artists already in the Navidrome library open the local artist page, while external-only artists open the native Aurral artist/catalog page with resolved artwork, missing albums, previews, and acquisition actions.
* Missing Aurral albums now open an album-like detail page instead of exposing a cramped request button in the artist carousel. The detail page resolves Aurral release-group cover JSON into a real cached image URL, sends Aurral Basic Auth headers only to Aurral-hosted image URLs, shows matching 30-second previews for that release group, keeps the bottom transition consistent with the now-playing gradient, and uses the primary action to trigger Aurral acquisition. Local album detail pages resolve Aurral recovery candidates through album title, split album-artist credits, and a near-year match window before falling back to title-only search, then fetch the full Aurral/Lidarr tracklist, infer disc sections when Aurral omits explicit disc numbers but track numbers reset, mark locally owned tracks by MBID or best-effort metadata, merge missing/requestable tracks into the normal track list with red/orange/green status dots instead of a separate `Album recovery` section, place local-only songs into gaps in the canonical Aurral track order when the Navidrome album only contains a partial subset, keep missing-state text out of the artist/subtitle slot, route lookup loading through the page-local Aurral integration indicator, and expose a dedicated Aurral-logo header action whenever unresolved Aurral tracks remain. Missing Aurral album actions show the crossed Aurral state instead of the normal acquisition icon.
...
* Artist detail routing resolves the optional Last.fm integration without requiring a separate injectable API client, generated fallback artwork shows a compact spinner while image/artwork resolution is still active, and Most played artist shortcuts reconcile stored artist ids against the current local artist table before navigation so stale playback-origin rows do not open missing artist records. Most played artist shortcuts persist verified artist photos, including authenticated Aurral artist images, into a local artist-photo cache, reuse that cache on artist headers, and ignore stale protected absolute Navidrome artist URLs that cannot be loaded without headers before falling back to played song or album artwork. Most played artist-photo resolution logs sanitized handoff and image-loader diagnostics under `MostPlayedArtwork` and `CoverArt` so unresolved fallback cases can be traced on-device without exposing auth header values.
* The Now Playing technical-info chip is rendered in the existing gap between controls and progress instead of the artwork slot or a new vertical row, so format/bitrate text no longer collides with square covers, rotating vinyl artwork, or the Up next rail. Landscape Now Playing also moves the collapse control into the track/action row beside star and more actions so it does not overlap the top-right buttons.
* Playback downloads now queue audio first and enrichment second. When playback is active, Navic prefetches the upcoming songs shown in `Up next` so large local downloads and metadata enrichment are less likely to stall the current flow.
* Page-local integration pulse strips show small pulsing marks only for integration work affecting the visible screen. Now Playing can pulse LidaClips, MusicBrainz, or Lyrics while verified-only actions resolve; the LidaClips player pulses while a clip loads; MusicBrainz info pulses during recording/artwork metadata resolution; Library pulses for Aurral discovery loading; and artist pages pulse for Aurral or Last.fm enrichment. If an enabled integration was attempted and the service is unavailable, the same strip keeps a frozen service mark with a red cross until a later successful attempt clears it. Disabling the integration hides that failed mark without clearing cached data.
* Disabled or incomplete integrations are gated at navigation, screen, repository, and queue boundaries: their rows, route contents, loading indicators, stale status rows, and new background jobs are suppressed while settings and explicit cache-clear actions remain available. Turning an integration off now notifies active screens and local managers so Aurral confirmation work, cached Aurral request/status rows, Last.fm artist enrichment, LidaClips playback/cache jobs, LidaClips Activity queue rows, and MusicBrainz fallback-artwork exposure are drained or hidden without deleting cached media, artwork, metadata, or saved settings.
* Integration pulse strips render transparent branded PNG assets instead of raw SVG resources through the Android painter path, and the Library pulse sits in the status/top-left overlay area instead of being pushed down into the content grid.
* Activity queue rows use compact retry, discard, cancel, and clear icon actions for Navidrome and LidaClips downloads. Hosted media downloads stay queued and retrying for local/cache-transfer errors; a row is marked failed only when the Navidrome or LidaClips service path appears unavailable. Static/fallback LidaClips assets are not exposed as verified music-video actions.
...
Use the normal Download action on songs, albums, playlists, stations, or artist pages. Navic marks pending songs as queued and routes every song through the same queue before any network download starts. Settings -> Data & Storage shows a `Download queue` count for queued or currently downloading songs; tap it to inspect queued, downloading, and failed rows, retry failed downloads that still exist locally, cancel pending downloads, or clear the whole active/failed queue without deleting completed downloads. The Activity tab mirrors those queue controls so stale rows can be retried, discarded, or cleared without navigating through Settings. Hosted Navidrome song downloads and LidaClips clip downloads stay queued and retrying for recoverable file/cache-transfer errors; `Failed` is reserved for service/API-path outages such as unreachable hosts, refused connections, timeouts, or 5xx responses. Use `Parallel downloads` to limit concurrent song downloads; the default is 3. Restart recovery resumes queued songs that still exist locally, but it does not yet preserve the original per-collection grouping after the app process is killed.
...
Open Settings -> Integrations -> `Last.fm`, enable the integration, and enter a Last.fm API key if you want artist pages to resolve most popular tracks in-app. Use `Test connection` to validate the key, then review `Service status` for the configured key state, integration state, native artist top-track support, and validation sample. Navic only uses the key for the artist top-tracks lookup, then matches those titles against locally synced songs so playback still uses your Navidrome library. Leave the key blank or disable the integration to hide the row.

### Bindery setup

Open Settings -> Integrations -> `Bindery`, enable the integration, enter the canonical OPDS 2 URL such as `https://bindery.example.com/opds`, and enter the Bindery API key. Navic sends the key as `X-Api-Key` to OPDS catalogs, manifests, covers, action endpoints, and audio resources. Use `Language filter` to choose the ISO language code used for owned catalog filtering; it defaults to `eng`, and a blank value or `all` disables language filtering. Use `Book grid columns` to control Bindery book/collection density from 5 to 8 columns; the setting defaults to 5 and does not affect music grids. Use `Test connection` to validate the OPDS root, then review `Service status` for OPDS URL/API-key state, root navigation count, audiobook, author, collection, findings, series, search, progress-sync, and pagination capability detection. OPDS catalogs, manifests, resource lists, and per-book findings are persisted locally, served fresh-first, and reused as stale fallback when Bindery is temporarily unavailable; successful OPDS action requests clear the affected Bindery metadata cache so newly acquired or monitored content can refresh. Current client work includes configuration, capability detection, first-party Audiobooks hub/Books/Findings/Collections/Authors browsing, selected-language availability filtering, contextual audiobook search that can include unowned books, findings, and discovered authors, native Book landing pages with compact quality-sorted audiobook/ebook Findings rows, native Finding detail pages with provider evidence and mapped-book/file metadata, advertised OPDS monitor actions for unowned authors/books plus download-request actions on concrete findings, dedicated Author and Collection detail pages, author collection rows from advertised OPDS navigation, collection publication ordering from OPDS properties, cleaned collection summaries that hide backend provider names, and a first-pass native audiobook player with Navic-styled artwork background, chapter index, speed selector, sleep timer, and fixed seek transport controls; provider-total collection membership, richer collection descriptions, ebook playback, and resume conflict handling remain future work.

Bindery availability dots are derived from selected-language OPDS availability, concrete acquisition resource links, and publication `readingOrder` resources. Findings are treated as source/download evidence, so an owned ebook without findings still appears as partial availability instead of disappearing or becoming a false missing book.

The exact Bindery server contract Navic needs next is tracked in [docs/superpowers/specs/2026-06-01-bindery-opds-client-contract.md](docs/superpowers/specs/2026-06-01-bindery-opds-client-contract.md).

### Aurral setup

Open Settings -> Integrations -> `Aurral`, enable the integration, enter your Aurral URL, and optionally enter the Aurral username/password if your backend requires auth. Use `Test connection` to verify the endpoint, then check `Service status` and `Acquisition queue` for health, Flow, discovery, Lidarr, and request diagnostics. Once configured, Library shows discovery collections below the local rows, including recently discovered artists, recent releases, recommendations, backend-provided fallback genre rails, generated genre rails, and tag chips; tap tag or genre entries to open a native artist grid filtered by that tag. The main Search screen also returns discovery artists and albums alongside Navidrome results. A recommended artist that already exists locally opens the native artist page, with album recommendations and monitor state resolved there from the local artist identity before weaker Discover matches. Open Artists and tap `+` beside sort to search external artists and optionally monitor them. The Aurral action toggles Lidarr monitoring through Aurral: monitoring uses Aurral's selected/default monitor option, marks eligible albums monitored, and only triggers Lidarr searches when the backend's search-on-add setting is enabled; unmonitoring sets the artist monitor option to `none` and leaves already downloaded albums/files in place. Because Aurral/Lidarr artist adds can be asynchronous, Navic keeps the action queued and visually pending until the library endpoint confirms the final state. Open Stations and tap `+` to create a new Aurral Flow that will sync back as a `[A]` Station after Navidrome refreshes. Artist pages with MusicBrainz artist ids or verified discovery matches can show local and missing albums in one deduped newest-first Albums row, monitoring, acquisition progress strips, request status, ownership/request status dots, and unified similar artists from this configured connection. Tap a missing album to open its album-like page with resolved cover art, 30-second previews when Aurral can match them, and an acquisition action. Album grids, song cards, and the Library recent-albums row can also show ownership/request status when local items match discovery/request state. Normal playlists expose delete from their detail `...` menu and selection sheet, while `[A]` Stations stay protected from playlist deletion.
...
5. Review `Service status` to see active, official, fallback, recent clips, backend health, and recent backend failure diagnostics. Client-side clip cache/download work appears in Activity as the LidaClips queue with retry, discard, cancel, and clear controls. The status refreshes after URL or API-key edits settle; use `Pause LidaClips sync` if you want the backend to stop scheduled clip processing.
