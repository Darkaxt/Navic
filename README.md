...
* Missing Aurral albums now open an album-like detail page instead of exposing a cramped request button in the artist carousel. The detail page resolves Aurral release-group cover JSON into a real cached image URL, sends Aurral Basic Auth headers only to Aurral-hosted image URLs, shows matching 30-second previews for that release group, keeps the bottom transition consistent with the now-playing gradient, and uses the primary action to trigger Aurral acquisition.
