...
* Artist pages can enrich local Navidrome artists with Aurral data when the artist has a MusicBrainz artist id: missing release groups, existing request status, small acquisition progress strips over requested and locally matched album covers, an eye action for monitoring the artist in Aurral, and one unified similar-artists row that merges Aurral image/match data with local Navidrome similar artists. Similar artists already in the Navidrome library open the local artist page, while external-only artists are shown muted.
...
Open Settings -> Integrations -> `Aurral`, enable the integration, enter your Aurral URL, and optionally enter the Aurral username/password if your backend requires auth. Use `Test connection` to verify the endpoint, then check `Service status` and `Acquisition queue` for health, Flow, discovery, Lidarr, and request diagnostics. Once configured, open `Aurral hub` from Library to see native Discover, Requests, and Flows summaries. Artist pages with MusicBrainz artist ids can show missing albums, monitoring, acquisition progress strips, request status, and unified similar artists from this configured connection. Tap a missing album to open its album-like page with resolved cover art, 30-second previews when Aurral can match them, and an acquisition action. Album grids can also show acquisition progress when a local album matches an Aurral request. The next Aurral slices are Flow/station creation and Flow playback.
