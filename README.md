...
* Artist pages can enrich local Navidrome artists with Aurral data when the artist has a MusicBrainz artist id: local albums and Aurral-only missing albums are merged into one newest-first Albums row, local albums win duplicate matches, missing albums use grayscale Aurral-resolved covers, existing request status is shown, small acquisition progress strips appear over requested and locally matched album covers, an eye action monitors the artist in Aurral after the current monitor state is confirmed, and one unified similar-artists row merges Aurral image/match data with local Navidrome similar artists. While Aurral monitor status is still pending, the eye action uses a disabled question-mark state instead of showing a false unmonitored state. Similar artists already in the Navidrome library open the local artist page, while external-only artists open the native Aurral artist/catalog page with Aurral artwork, missing albums, previews, and acquisition actions.
