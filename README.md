...
* LidaClips matches are cached locally before Now Playing video features activate. Once cached, clips can play as a muted cropped Now Playing video background by default; the movie button beside Lyrics promotes the clip into the artwork area, starts it at the equivalent song-progress percentage, mutes the underlying song, and lets the video provide audio while Navic's normal controls remain visible.
...
* Now Playing background and promoted artwork video only activate after the matching clip has been fully cached into Navic's temporary LidaClips video cache. The `Video cache size` setting defaults to 512 MB, can be turned off, and is separate from offline song downloads.
