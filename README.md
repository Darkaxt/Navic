...
* Optional LidaClips integration for matching current Navidrome tracks to music-video clip streams, including cached now-playing lookups, automatic hiding for known missing clips, retryable playback errors, Feishin-style music pause/resume while clips play, and Android Picture-in-Picture
...
5. Optionally enable `Picture-in-Picture` on Android. `Pause music while clips play` is on by default and can be turned off if you want music under the clip audio.
