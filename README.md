...
5. Review `Service status` to see active, official, fallback, recent clips, backend health, recent backend failure diagnostics, and any exposed clip download queue. The status refreshes after URL or API-key edits settle; use `Pause LidaClips sync` if you want the backend to stop scheduled clip processing.
