...
* The Library screen can surface a local `Most played` shortcuts row for artists, genres, albums, playlists, and `[A]` stations that were explicitly played from Navic. Shortcuts are ranked by effective playback duration instead of raw taps, open the native destination for that entity, and hydrate artist shortcut photos from verified artist-image sources before falling back to local song or album artwork instead of showing generated placeholders. Resolved artist-photo URLs are persisted in the local cache database, so Most played artist cards can reuse them after app restarts instead of waiting for integration lookups each time.
