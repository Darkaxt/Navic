...
Open Settings -> Integrations -> `Bindery`, enable the integration, enter the canonical OPDS 2 URL such as `https://bindery.example.com/opds`, and enter the Bindery API key. Navic sends the key as `X-Api-Key` to OPDS catalogs, manifests, covers, action endpoints, and future audio resources. Use `Language filter` to choose the ISO language code used for owned catalog filtering; it defaults to `eng`, and a blank value or `all` disables language filtering. Use `Book grid columns` to control Bindery book/collection density from 5 to 8 columns; the setting defaults to 5 and does not affect music grids. Use `Test connection` to validate the OPDS root, then review `Service status` for OPDS URL/API-key state, root navigation count, audiobook, author, collection, series, search, progress-sync, and pagination capability detection. Current client work includes configuration, capability detection, first-party Audiobooks hub/Books/Collections/Authors browsing, selected-language availability filtering, contextual audiobook search that can include unowned books and discovered authors, native Book landing pages with compact quality-sorted audiobook/ebook availability rows, advertised OPDS monitor/download-request actions for unowned authors and books, dedicated Author and Collection detail pages, author collection rows from advertised OPDS navigation, collection publication ordering from OPDS properties, and cleaned collection summaries that hide backend provider names; native Audible-like playback, chapter navigation, provider-total collection membership, richer collection descriptions, and resume conflict handling remain future work.
