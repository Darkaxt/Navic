...
* The bottom `Activity` tab centralizes live integration work across Navic song downloads, Aurral album acquisitions and Flow track states, and LidaClips recent failures or failed health checks. It can retry or discard failed Navidrome download rows, and Aurral acquisition rows expose cancel/retry actions when the backend provides enough album or artist identifiers.
