...
* LidaClips settings show backend service counts, recent backend sync failures, and can pause or resume scheduled backend clip syncs.
...
5. Review `Service status` to see active, official, fallback, and recent backend failure diagnostics. Use `Pause LidaClips sync` if you want the backend to stop scheduled clip processing.
