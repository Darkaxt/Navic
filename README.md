...
* Artist pages include a dedicated Shuffle action beside Play. Last.fm biographies expand and collapse inline, while the external Last.fm page stays in the secondary overflow action. If a Last.fm API key is configured, artist pages can show a local-song `Last.fm top tracks` row after `Frequently played`.
...
* Artist pages enrich local Navidrome artists with Aurral data by trying the local MusicBrainz artist id before weaker Aurral Discover name matches, but they fall through to verified Discover identities when the first candidate cannot confirm monitoring state. Artist headers prefer verified Aurral imagery before stale fallback-style local covers, then use the synced artist image URL. Local albums and Aurral-only missing albums are merged into one newest-first Albums row, local albums win duplicate matches, missing albums use grayscale Aurral-resolved covers, existing request status is shown, small acquisition progress strips appear over requested and locally matched album covers, and Aurral album/preview cards show compact status dots: green for owned, orange for partial/requested, and red for missing/unrequested. An eye action monitors the artist in Aurral after the current monitor state is confirmed, and one unified similar-artists row merges Aurral image/match data with local Navidrome similar artists. Monitor confirmation checks Aurral's library artist endpoint as a fallback, publishes a verified positive eye state before slower discovery work finishes, and no longer replays the whole artist-page transition when only the eye state changes. Similar artists already in the Navidrome library open the local artist page, while external-only artists open the native Aurral artist/catalog page with Aurral artwork, missing albums, previews, and acquisition actions.
...
Open Settings -> Integrations -> `Aurral`, enable the integration, enter your Aurral URL, and optionally enter the Aurral username/password if your backend requires auth. Use `Test connection` to verify the endpoint, then check `Service status` and `Acquisition queue` for health, Flow, discovery, Lidarr, and request diagnostics. Once configured, Library shows Aurral Discover collections near the top discovery area after Quick Picks/Most played, including recently added artists, recent releases, recommendations, backend-provided fallback genre rails, generated genre rails, and tag chips; tap tag or genre entries to open a native Aurral artist grid filtered by that tag. The main Search screen also returns Aurral artists and albums alongside Navidrome results. A recommended artist that already exists locally opens the native artist page, with Aurral album recommendations and monitor state resolved there from the local artist identity before weaker Discover matches. Open Artists and tap `+` beside sort to search Aurral artists and optionally monitor them. Open Stations and tap `+` to create a new Aurral Flow that will sync back as a `[A]` Station after Navidrome refreshes. Artist pages with MusicBrainz artist ids or verified Aurral discovery matches can show local and missing albums in one deduped newest-first Albums row, monitoring, acquisition progress strips, request status, ownership/request status dots, and unified similar artists from this configured connection. Tap a missing album to open its album-like page with resolved cover art, 30-second previews when Aurral can match them, and an acquisition action. Album grids and the Library recent-albums row can also show acquisition progress when a local album matches an Aurral request. Normal playlists expose delete from their detail `...` menu and selection sheet, while `[A]` Stations stay protected from playlist deletion.
