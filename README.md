...
* When a matching LidaClips clip is found for a song that is already downloaded for offline playback, Navic saves that clip into persistent app storage and prefers it before the temporary cache or network stream. Deleting a downloaded song clears that song's persistent clip files; clearing all downloads clears all persistent LidaClips offline clips.
...
* The native Aurral hub is reachable from Library when Aurral is enabled/configured. Settings -> Integrations keeps one `Aurral` configuration entry so setup and the hub are no longer duplicated.
...
* The native Aurral hub now loads Aurral Discover recommendations from `/api/discover`, has native Aurral artist search backed by `/api/search/artists`, has native Aurral album search backed by `/api/search?scope=album`, shows compact recommended/global/search rows with Aurral-resolved artwork and recommendation/catalog context, opens artists into a native Aurral artist/catalog page, and opens album search results into the same album-like missing-album detail page used by artist catalogs. Album search results are deduped and sorted by release year from newest to oldest, with undated results last. The hub can also send artists to Aurral monitoring when the configured Aurral user has artist-add permission, create new Flows, enable or disable existing Flows, trigger a Flow run when the backend exposes the required permissions, play the matching synced Navidrome `[A]` Station directly when it has songs, play ready Flow jobs directly from Aurral while the Station has not synced yet, and jump to that Station detail page when one already exists locally.
* Artist pages can enrich local Navidrome artists with Aurral data when the artist has a MusicBrainz artist id: local albums and Aurral-only missing albums are merged into one newest-first Albums row, local albums win duplicate matches, missing albums use grayscale Aurral-resolved covers, existing request status is shown, small acquisition progress strips appear over requested and locally matched album covers, an eye action monitors the artist in Aurral, and one unified similar-artists row merges Aurral image/match data with local Navidrome similar artists. Similar artists already in the Navidrome library open the local artist page, while external-only artists open the native Aurral artist/catalog page with Aurral artwork, missing albums, previews, and acquisition actions.
...
Open Settings -> Integrations -> `Aurral`, enable the integration, enter your Aurral URL, and optionally enter the Aurral username/password if your backend requires auth. Use `Test connection` to verify the endpoint, then check `Service status` and `Acquisition queue` for health, Flow, discovery, Lidarr, and request diagnostics. Once configured, open `Aurral hub` from Library to search Aurral artists or albums, see native Discover artist recommendations, Requests, and Flows summaries; tap a search or Discover artist to browse its Aurral catalog; tap an album search result to open the native missing-album page; monitor artists from the hub when your Aurral user can add artists; create new Flows; enable or disable Flows; start a Flow run when your backend account exposes those actions; play ready Flow jobs directly from Aurral before Navidrome has scanned the matching Station; and open the matching Navidrome Station once the `[A]` playlist has synced locally. Artist pages with MusicBrainz artist ids can show local and missing albums in one deduped newest-first Albums row, monitoring, acquisition progress strips, request status, and unified similar artists from this configured connection. Tap a missing album to open its album-like page with resolved cover art, 30-second previews when Aurral can match them, and an acquisition action. Album grids and the Library recent-albums row can also show acquisition progress when a local album matches an Aurral request.
