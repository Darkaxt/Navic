...
* The native Aurral hub now loads Aurral Discover recommendations from `/api/discover`, shows compact recommended/global artist rows with Aurral-resolved artwork and recommendation context, opens discovered artists into a native Aurral artist/catalog page, and can send discovered artists to Aurral monitoring when the configured Aurral user has artist-add permission. It can also create new Flows, enable or disable existing Flows, trigger a Flow run when the backend exposes the required permissions, play the matching synced Navidrome `[A]` Station directly when it has songs, play ready Flow jobs directly from Aurral while the Station has not synced yet, and jump to that Station detail page when one already exists locally.
