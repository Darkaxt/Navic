...
class ReaderRuntimeImageLinkTest {
	...
	fun commonReaderUsesNativeShellCoverSurfaceWhenResolverProvidesCoverUrl() {
		...
		assertContains(openRequestText, "val hasShellCover = !skipNativeShellCover && !shellCoverUrl.isNullOrBlank()")
		...
		assertContains(nativeFrameHostText, "readerNativeShellCoverSelected(")
		assertContains(nativeFrameHostText, "shellCoverDecision = presentationDecision")
		assertContains(nativeFrameHostText, "hasValidatedNativePredecessor =")
		assertFalse(
			nativeFrameHostText.contains(
	...
	fun androidReaderInjectsThemeColorsIntoPublicationDocuments() {
		...
		assertContains(bridgeText, "applyDocumentTheme(doc, settings = this.readerSettings, _index = undefined)")
		assertContains(bridgeText, "applyReaderContentDocumentRenderProfile(doc, settings)")
